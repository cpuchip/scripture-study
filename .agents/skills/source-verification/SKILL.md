...
- `search_scriptures` / `search_talks` for concept search
...
- For EVERY scripture you plan to quote, `view_file` the actual chapter markdown from `gospel-library/`
- For EVERY conference talk you plan to cite, `view_file` the actual talk file
...
For **N** citations in your output, perform at least **N** tool reads on actual source files.

If you cite 8 scriptures and 2 conference talks, you should have called `view_file` (or equivalent tool) at least 10 times on actual source files.
...
|------------------------------|---------------------------------------|
| `view_file` on a gospel-library markdown file | `gospel_search` excerpts |
| `view_file` on a conference talk file | `search_scriptures` summaries |
| `view_file` on a manual chapter | `search_talks` summaries |
...
After writing the study, include a **Becoming** section. Every study should land somewhere personal — not just knowledge, but direction.
...
- [ ] No quotes generated from memory — every `"quoted text"` has a corresponding `view_file` call
...
- [ ] Files claimed to exist are verified with `list_dir` or `view_file`
...
- [ ] `view_file` was used at least once per cited source (the cite-count rule)
