...
knowledge per intent (Vivint, Marsfield, …) in Postgres. An `okf_export(intent)` could emit a bundle: one
...
- **Build it when there's a concrete need** — the reflect-steward wanting to publish a Vivint pool to the
