-- =====================================================================
-- cv7 (2026-06-04) — the code-pr `review` critic stage is IMMUNE to the
-- work_item's model_override.
--
-- For the model bake-off we set work_items.model_override to the DEV model
-- (kimi-k2.6 / minimax-m3 / glm-5.1 / deepseek-v4-pro) so every dev stage
-- (clone/plan/implement/verify) runs on it. But model_override resolves for
-- ALL stages — which would turn the critic into the dev model judging its own
-- work, defeating the constant fresh-eyes critic. This adds one tightly-gated
-- branch: for pipeline_family='code-pr' stage='review', v_model is forced to
-- the stage's own model (e.g. qwen3.7-max), ignoring model_override. Every
-- other stage/pipeline is unchanged. Body below is the LIVE def (r3) verbatim
-- plus that branch.
-- =====================================================================
...
BEGIN
    ...
    -- cv7: the code-pr `review` critic stage ignores model_override and always
    -- uses its own stage.model (the constant fresh-eyes critic). So a bake-off
    -- model_override that sets the DEV model never turns the critic into the
    -- dev model judging its own work.
    IF v_wi.pipeline_family = 'code-pr'
       AND v_wi.current_stage = 'review'
       AND (v_stage->>'model') IS NOT NULL THEN
