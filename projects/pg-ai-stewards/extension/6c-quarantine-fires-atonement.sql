-- =====================================================================
-- Batch G.3 — Quarantine fires Atonement
--
-- Phase D shipped stewards.maybe_enqueue_atonement(work_item_id) which
-- enqueues an atonement chat IF the pipeline has atonement_enabled=true,
-- otherwise no-ops. Nothing called it.
--
-- This commit re-creates steward_tick (preserving G.2's
-- retry_guidance_with_lessons swap) with one addition at the
-- cost-cap-exceeded quarantine point:
--
--   PERFORM stewards.maybe_enqueue_atonement(v_item.id);
--
-- Safe to add unconditionally — maybe_enqueue_atonement is a no-op
-- when pipeline.atonement_enabled is false.
--
-- Phase 4a follow-up gap (not addressed here): failure_count >= 3
-- rows are EXCLUDED from steward_tick processing via the query
-- filter, so they never get quarantined_at set. Phase 4a should
-- have set quarantined_at + quarantine_reason='failure_count_limit'
-- when failure_count hits the threshold. That's a separate fix;
-- this commit doesn't expand G.3's scope.
-- =====================================================================
    ...
    LOOP
        BEGIN
            ...
            IF stewards.cost_cap_exceeded(v_item.id) THEN
                ...
                -- Batch G.3: fire atonement on quarantine. No-op if
                -- pipeline.atonement_enabled is false.
                PERFORM stewards.maybe_enqueue_atonement(v_item.id);
...
'Phase 4d + Batch G.2/G.3 (2026-05-11): retry composer pulls last 3 ratified lessons (G.2); cost-cap-exceeded quarantine fires maybe_enqueue_atonement (G.3, no-op if pipeline.atonement_enabled=false).';
