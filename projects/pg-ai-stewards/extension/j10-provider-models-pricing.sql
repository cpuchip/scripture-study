...
VALUES
    ...
    ('opencode_go', 'qwen3.7-max',          2500000,  7500000,        NULL, 500000,
     '2026-05-29 00:00:00+00', 'UNUSABLE via substrate (2026-05-29): gateway rejects with ''not supported for format oa-compat''. Appears in catalog but cannot dispatch on the OpenAI-compat endpoint.'),
    ('opencode_go', 'qwen3.5-plus',          200000,  1200000,        NULL,  20000,
     '2026-05-29 00:00:00+00', ''),
    ('opencode_go', 'glm-5',                1000000,  3200000,        NULL, 200000,
     '2026-05-29 00:00:00+00', 'Reasoning model (backend frank/GLM-5.1). Non-streaming works with ample max_tokens; STREAMING returns empty content via the gateway (2026-05-29). Substrate dispatch streams (stream:true), so it comes back empty. Avoid for substrate lenses until gateway streaming is fixed.'),
