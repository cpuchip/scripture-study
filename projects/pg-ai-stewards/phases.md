...
> | **3f v1 phase 1** | Local-first single-user UI scaffold landed 2026-05-09 evening. Decisions ratified: name `stewards-ui`, separate `ui` compose service, single port 8080 (Go serves Vue dist + /api/*), Vue 3 + Vite + Tailwind 4 + vue-router (matches becoming-app), shadcn-vue components, Cytoscape.js graph in v1. **Phase 1 = foundation only**: Go binary with embed.FS skeleton, Vue/Vite project, multi-stage Dockerfile (node-build → go-embed → alpine runtime), `ui` compose service depends on pg.healthy. Verified live: container up, /healthz pings db, / serves real Vite-built Vue app, /studies SPA-fallback works, /api/* returns 501 as designed. Phases 2-7 (real /api endpoints + view content + graph) follow in subsequent sessions. → [proposal](../../.spec/proposals/pg-ai-stewards-3f-local-ui.md) · [build plan](../../.spec/scratch/stewards-ui-build-plan-2026-05-09.md) | **shipped 2026-05-09 (foundation)** |
