...
> | **3c.3.1** | Fix the 3 bugs from 3c.3 v1: (a) `v_is_final` NULL handling in trigger; (b) `_work_item_id` markers don't propagate to continuation chats so the trigger only sees the FIRST chat per stage; (c) per-agent `steps` budget for plan/study agents needs to be much larger than the default 8 to allow real research. | not started |
> | **3c.3.2** | Multi-stage pipeline run after 3c.3.1 — re-run study-write on the same binding question, expect actual synthesis output. Goal: a real meta-study from the substrate. | not started |
