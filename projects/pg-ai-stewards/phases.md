...
> | **3c.3.6** | Full split of monolithic `extension/src/lib.rs` (was 4246 lines, single file) into 6 modules: `lib.rs` (246), `schema.rs` (2432), `bgworker.rs` (1041), `tools.rs` (418), `providers.rs` (127), `types.rs` (64). All four pgrx-macro-visibility risks (\#\[pg\_guard\], `extension_sql!`, `extension_sql_file!`, `#[pg_extern]`) resolved by research into real production extensions (pg\_vectorize, ParadeDB pg\_search) — captured authoritatively in `.github/skills/pgrx-rust/SKILL.md`. Build clean after each move; final smoke test on fresh DB confirmed all 30 SQL entities still emit. Live container restarted onto the modular image; substrate state preserved. **Total session time ~1 hour vs 3-5h estimated** — front-loaded research collapsed the uncertainty. → [findings doc](docs/lib-rs-refactor-findings.md) | **shipped 2026-05-08 (v5 final)** |
...
> | **3d v1** | Sandboxed git/gh MCP wrapper. New `scripts/git-mcp/` Go binary. 8 tools (git_clone/status/branch_create/add/commit/push, gh_pr_create/issue_create) with allow-list discipline: branch namespace locked to `agent/<pipeline>/<work-item-id>-<slug>`, protected branches refused, --force never sent, GITHUB_TOKEN env-only (never in tool args), Co-Authored-By trailer auto-appended (`<agent-family>-via-pg-ai-stewards <agents@cpuchip.net>`), workdir kept on disk for inspection. Built into bridge image at /usr/local/bin/git-mcp; git + gh CLIs added to alpine runtime. Granted to study agent (7 of 8 tools; gh_issue_create deferred). PAT setup deferred per Michael; live test triggers when GITHUB_TOKEN lands in .env. → [proposal](../../.spec/proposals/pg-ai-stewards-3d-sandboxed-git.md) | **shipped 2026-05-09** |
> | **3e.1 v1** | MCP server sidecar — `cmd/stewards-mcp/` Go binary using the official Anthropic+Google `modelcontextprotocol/go-sdk` v1.6.0 (released same day). Connects to substrate via pgxpool, speaks MCP over stdio. Two read-only tools: `study_search` (FTS over substrate corpus) and `study_get` (line-paginated body read). Manual smoke test verified end-to-end: initialize handshake, tools/list, tools/call all clean. Configured in `.mcp.json` (gitignored locally; entry shape documented in `cmd/stewards-mcp/README.md`). New skill `.github/skills/mcp-server-go/SKILL.md` captures patterns. → [findings doc](docs/3e-mcp-findings.md) | **shipped 2026-05-08** |
> | **3e.1.1** | Polish: fixed jsonschema struct-tag syntax (jsonschema-go reserves `WORD=` prefixes for future use; constraints aren't supported via tags — use plain prose). Added `study_similar` + `study_citations`. Live-tested against gadianton-robbers: 35 citations including Ether 8-9, Helaman 6:38-39, Moses 5:51. study_similar returns 0 because substrate similarity edges aren't populated yet — separate Watchman 2.x graph work. | **shipped 2026-05-08** |
> | **3e.2.a v1** | Outbound MCP-client substrate. `stewards.mcp_servers` registry + `stewards.mcp_tool_cache` schemas with seed rows for the 7 known external MCP servers (gospel-engine-v2, webster, yt, byu-citations, becoming, search, exa-search). New `stewards-mcp bridge refresh-tools` subcommand uses the official Go MCP SDK as a client (`CommandTransport` for stdio, `StreamableClientTransport` for HTTP). 6/7 servers responded with their tool catalogs (40 tools cached); search-mcp had a `tools/list: invalid request` protocol regression noted for follow-up. Concurrency: `_PG_init` now registers N (default 4, max 16) dispatcher workers in a leader-gated pattern (worker 0 owns reaper + Watchman scheduler tick; all workers share the claim loop via FOR UPDATE SKIP LOCKED). | **shipped 2026-05-08** |
> | **3e.2.b/c v1** | Async fan-out + bridge daemon. Substrate side (3e.2.b): `work_queue.status` gains `'waiting_for_tools'`; new `stewards.mcp_proxy_enqueue()` SQL fn inserts a child `kind='mcp_proxy'` row + NOTIFY's the bridge; new `stewards.tool_dispatch_complete_waiting()` is the completion pass run from each bgworker tick (concurrency-safe via SKIP LOCKED). Rust side: `ToolReply::Sync/Async` enum + new `WorkOutcome::WaitingForTools` arm; bgworker claim/reaper filter `kind <> 'mcp_proxy'`. 3 example tool_defs seeded for `gospel_search`/`gospel_get`/`webster_define`, **deny-by-default** so the existing soak surface is unchanged. Sidecar side (3e.2.c): `stewards-mcp bridge run` long-running daemon — LISTEN+claim+dispatch with N workers, lazy per-server MCP session cache, stale-row reaper, graceful shutdown. End-to-end verified: synthetic `mcp_proxy_enqueue` → bridge claim within 2.4ms → 3 real `gospel_search` results in 258ms total, `isError=false`. | **shipped 2026-05-08** |
> | **3e.2.d/e v1** | Auto-promote + crash recovery. `stewards.promote_mcp_tool_cache_to_tool_defs()` SQL fn + AFTER trigger on `mcp_tool_cache` keep `tool_defs` in sync (40 cached tools → 40 mcp_proxy tool_defs). Bridge `sessionCache.invalidate(name)` drops dead sessions on CallTool transport errors so the next call lazy-reinits. 12 manual `agent_tool_perms` grants (study/lesson/talk × 3 gospel tools, journal/review × gospel_search, research × web_search_exa). | **shipped 2026-05-08** |
> | **fetch-md-mcp v1** | New Go MCP server at `scripts/fetch-md-mcp/` (Mozilla Readability + html-to-markdown, 4 tools: `fetch_url`, `fetch_urls`, `extract_links`, `fetch_url_raw`). Registered as 8th external MCP server. Plain HTTP only — JS rendering deferred. Granted to `research` agent alongside `web_search_exa` for v1 research surface outside the canon. | **shipped 2026-05-08** |
> | **fetch-md v2** | JS rendering via chromedp + alpine chromium. Optional `js: true` and `wait_ms` params on all 4 tools (default false; existing fast HTTP path unchanged). When true, launches headless Chromium, navigates, settles, extracts rendered DOM. Bridge image gains chromium package (~150MB). Verified end-to-end: Wikipedia render in 3.3s. | **shipped 2026-05-09** |
> | **bridge-in-docker** | Replaces host-shell `stewards-mcp bridge run` with a docker-compose service. New `bridge` service uses bridge.Dockerfile (multi-stage Go build → alpine runtime with binaries at /usr/local/bin/ + ca-certs + git + gh + chromium). Brings up after pg-healthcheck; `restart: unless-stopped`; tokens via .env. mcp_servers.command paths migrated to Linux (3e2-6). 9 of 9 servers respond after rebuild. Verified through Docker Desktop update cycle: WAL recovery clean, retry-on-DNS-settle worked. | **shipped 2026-05-09** |
> | **3e QoL sweep** | (a) Fixed search-mcp `tools/list: invalid request` from 3e.2.a — JSON-RPC 2.0 violation in handler default arm, plus missing `notifications/initialized` case. Same defensive fix audited across 5 hand-rolled MCP servers (yt-mcp, gospel-engine, gospel-mcp, gospel-vec, gospel-engine-v2). (b) 45 new agent grants opening read-only access across the bridge (study/lesson/talk/journal/review/research/yt-gospel — write tools deny-by-default). (c) Resolved `[slug](#)` link placeholders in mysteries-of-god study; new `scripts/study-export/` Go helper automates the resolution for future substrate-produced studies. **First substrate-bridge organic e2e** — mysteries-of-god study made dozens of `gospel_get` calls during review-stage quote verification. | **shipped 2026-05-08/09** |
> | **3e.4 v1** | Read-only inbound substrate inspection tools: `work_item_list`, `work_item_show`, `watchman_passes_list`, `watchman_pass_show`. New `inspection.go` module. Lets Claude Code triage substrate state without `docker exec`. Live-tested: returns expected 3 voice-experiment work_items + recent soak passes with verdict_counts. `stewards_brain` dropped from spec (substrate's brain_entries table has 1 row; v3 corpus consolidated into studies during v3→v4 migration). | **shipped 2026-05-08** |
> | 3e.4 v2 | Write-mutating inbound tools: `work_item_create`, `work_item_dispatch`, `work_item_advance`, `watchman_pass_now`. Defer until v1 read-only proves out and we know what's missing. Cost guardrail: substrate's `token_budget` per work_item bounds blast radius. | not started |
> | 3e.5 | `gospel_passthrough` inbound — wraps an outbound call to gospel-engine-v2. Easy if 3e.2 ships first. | not started |
> | **3f v1 (all 7 phases)** | Local-first single-user UI shipped 2026-05-09 evening end-to-end. Phase 1 foundation: Go binary with embed.FS, Vue 3 + Vite + Tailwind 4 + vue-router scaffold, multi-stage Dockerfile, `ui` compose service. Phases 2-7 in one push: backend `scripts/stewards-ui/api/` (8 handler files: dashboard with parallel-fan-out queries, studies list/get/search wrapping substrate's FTS+similar+citations SQL fns, work_items list/get with filters, sessions get with message timeline, watchman passes/pass with verdicts, bridge state from mcp_bridge_state view + per-server tool catalog, POST /api/work-items/create wrapping work_item_create + dispatch_stage, graph studies-citations subgraph). Frontend: 9 view components (Dashboard cards w/ 5s polling, Studies+StudyDetail w/ markdown-it + Tailwind typography, WorkItems+WorkItemDetail, Sessions w/ role-tinted timeline, Watchman pass list w/ verdict badges, BridgeState w/ expandable tool catalog, NewWork form, Graph w/ Cytoscape cose-layout). All 10 endpoints + SPA fallback verified post image-rebuild. Open http://127.0.0.1:8080. → [proposal](../../.spec/proposals/pg-ai-stewards-3f-local-ui.md) | **shipped 2026-05-09** |
