...
> | **3c.3** | First real multi-stage pipeline (e.g., `study-write`: outline → sources → critical-analysis → draft → review). End-to-end verification with the imported agent corpus. **Now blocked only on 3c.2.5 (study tools); 3c.2 shipped.** | not started |
