...
> | **3c.3.6** | Split monolithic `extension/src/lib.rs` (4246 lines, single file, no module structure) into proper Rust modules: `lib.rs` (top-level + extension_sql_file! macros), `schema.rs` (DDL blocks), `bgworker.rs` (tick loop + dispatch + chat/embed), `tools.rs` (tool dispatch helpers), `providers.rs` (registry + types). Compile-time and navigation cost was getting noticeable; 3e (MCP) work would push it past 5000 lines. Soak paused for the duration. Findings tracked at `projects/pg-ai-stewards/docs/lib-rs-refactor-findings.md` for digestion later. | **in flight** (2026-05-08) |
