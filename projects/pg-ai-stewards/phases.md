...
> | **3c.3** | First real multi-stage pipeline (e.g., `study-write`: outline → sources → critical-analysis → draft → review). End-to-end verification with the imported agent corpus. **Unblocked — 3c.2 + 3c.2.5 both shipped.** | not started |
