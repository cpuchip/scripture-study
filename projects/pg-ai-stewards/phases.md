...
> | **3c.3.5** | (was 3c.3.3 speculative) auto-promote completed `work_items` into `stewards.studies` rows so future Watchman passes can graph-walk pipeline-produced studies | not started |
> | **3c.4** | gospel-engine-v2 HTTP tool registration. **Deferred 2026-05-08** — neither `pg_net` (Supabase) nor `http` (pramsey) extensions ship with the `pgvector/pgvector:pg18` base; only `plpgsql` is registered. Building either against PG18 from source in autonomous mode was too risky. Two paths for daytime: (a) extend Dockerfile with a build stage for `pg_net` or `pgsql-http`; (b) extend the Rust bgworker with a `tool_http` work_kind using reqwest. Path (a) keeps bgworker untouched; path (b) avoids extension toolchain risk. | not started |
> | **agent_tool_perms provenance** | (followup from 3c.3.3) Add `source text` column to `agent_tool_perms` (`'frontmatter'` / `'broadcast'` / `'manual'`). Importer's `DELETE WHERE agent_family=$1` only deletes `source='frontmatter'` rows. Substrate-internal broadcasts survive reimports without being declared in agent frontmatter. | not started |
