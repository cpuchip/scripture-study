...
> | 3e.1.1 | Polish: fix jsonschema struct-tag syntax to emit minimum/maximum constraints; add `study_similar` + `study_citations` tools. Mechanical. | not started |
...
> | 3e.4 | Inbound tools: `stewards_brain`, `stewards_work_item`. Mechanical extension of 3e.1 pattern (~50 lines per tool). | not started |
