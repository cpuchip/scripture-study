...
> | 3e.2 | Outbound HTTP path (the former 3c.4) — substrate-internal agents reaching gospel-engine-v2 for quote verification. Design choice between extending the Rust bgworker (touches soak-critical code) vs adding MCP-client to the sidecar (cleaner but unfamiliar dispatch arch). Needs focused design session. | not started |
