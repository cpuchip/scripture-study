...
> | **3c.3.3** | Importer `model_match` extension — `cmd/stewards-cli/internal/importer/agents.go` reads `model_match` from frontmatter (falls back to `'*'`), so per-model agent variants (e.g., kimi-tuned study prompt at `.stewards/kimi-k2.6/study.agent.md`) can be imported without overwriting the base. Foundation for the multi-model voice experiments. | **in flight** (2026-05-08 overnight session) |
> | **3c.3.4** | Multi-model voice experiment — re-run FtC/WtL binding question on (a) kimi-k2.6 with kimi-tuned prompt and (b) qwen3.6-27b with base prompt. Compare against the 3c.3.2 original to evaluate whether per-model prompt tuning fixes voice signatures, and to discover qwen-specific signatures for future tuning. | **in flight** (2026-05-08 overnight session) |
...
> | **3c.4** | gospel-engine-v2 HTTP tool registration via `pg_net` (no bgworker change). Adds `execute_target='http_proxy'` and `tool_http_dispatch()` SQL helper. Registers `gospel_search` + `gospel_get` and grants to study + stewards-explore. Unblocks real scripture-quote verification — addresses the confabulation-under-audit failure mode from the 3c.3.2 review. | **in flight** (2026-05-08 overnight session, stretch) |
