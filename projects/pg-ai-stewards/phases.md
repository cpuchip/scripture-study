...
> | 3e.2.d | Auto-promote `mcp_tool_cache` rows into `stewards.tool_defs` with deny-by-default `agent_tool_perms`. Today's three example tool_defs are hand-curated; this phase makes the cache → registry path generic and idempotent. | not started |
