...
| ④ | [`substrate-coding-capability.md`](proposals/substrate-coding-capability.md) | the substrate writes/builds/tests/deploys code — native `coder` MCP + hardened-container/worktree sandbox + `code-write` pipeline (build/test = ground-truth verify gate) + deploy-to-sidecar behind the always-escalate Hinge | **DRAFT 2026-06-03.** 3 core decisions ratified (D-CC1 native tools · D-CC2 hardened container + worktree, switchable net default-on/offline · D-CC3 deploy in v1 behind the Hinge). D-CC4–D-CC8 open. Builds the always-escalate rung from `docs/delegation-pattern-skills-and-gates.md`. Researched vs. opencode + the 2026 sandbox literature. |
