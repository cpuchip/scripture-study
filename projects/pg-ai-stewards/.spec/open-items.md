...
| ④ | [`substrate-coding-capability.md`](proposals/substrate-coding-capability.md) | the substrate writes/builds/tests/deploys code — native `coder` MCP + hardened-container/worktree sandbox + `code-write` pipeline (build/test = ground-truth verify gate) + deploy behind the always-escalate Hinge | **✅ RATIFIED 2026-06-03, build-ready (not built).** D-CC1 native coder tools · D-CC2 hardened container + worktree (medium-safe, shared kernel accepted) · D-CC3 deploy in v1 behind Hinge · D-CC4 Go+Node/TS+Python image · D-CC5 egress open default-on/offline · D-CC6 deploy BOTH phased (v1 local sidecar, v2 Hinge-gated Dokploy w/ scoped access) · D-CC7 loop free, trust-gate at PR · D-CC8 sandbox-mgr in coder-mcp. 6-phase plan CC.1–CC.6 (+ CC.7 v2 Dokploy). Builds the always-escalate rung from `docs/delegation-pattern-skills-and-gates.md`. Next: CC.1 (coder-runtime image + sandbox-mgr + bridge docker-socket). |
