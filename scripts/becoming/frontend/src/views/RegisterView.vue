...
// Check for OAuth errors and provider availability
onMounted(async () => {
  if (route.query.error) {
    error.value = 'Sign-up with Google failed. Please try again.'
  }
  try {
    const providers = await authApi.providers()
    googleEnabled.value = providers.google
  } catch {
    // Ignore — just don't show Google button
  }
})
    ...
    <div class="w-full max-w-sm">
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
        ...
        <template v-if="googleEnabled">
          <button
            @click="googleSignUp"
            class="w-full py-2 px-4 bg-white border border-gray-300 rounded-lg font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 flex items-center justify-center gap-2"
          >
          ...
          <div class="relative my-6">
            ...
            <div class="relative flex justify-center text-sm">
              <span class="bg-white px-4 text-gray-400">or with email</span>
            </div>
          </div>
        </template>
