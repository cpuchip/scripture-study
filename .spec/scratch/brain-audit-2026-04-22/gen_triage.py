...
RECS = {
    # ============ VERIFIED — ideas without proposals (Phase D) ============
    '5d7d781a': ('done',     'Bug: captured widgets not showing — already fixed in WS2 Brain UX QoL P1-7b shipped Apr 6'),
    '17735051': ('archived', 'Context-Mode link only, no analysis — cost-prohibitive to investigate now'),
    '17748767': ('archived', 'Agent sandbox link, interesting but we have our own pipeline — defer indefinitely'),
    'a249be5d': ('promote',  'Brain app task structure — concrete UX idea worth a one-liner proposal in WS2'),
    ...
    '17752229': ('promote',  'Brain research pre-step pipeline — concrete, aligns with current direction'),
    '17752826': ('someday',  'AI personal assistant — too broad, revisit when scope narrows'),
    '17753137': ('archived', 'Use existing kanban tools — we chose to build, decision made; capture as decision elsewhere'),
    '17753205': ('archived', 'Brain as agent OS — already covered by current architecture direction'),
    '17753587': ('promote',  'Mount DB as filesystem — natural fit for brain-vscode-bridge Phase 2+'),
    '17733627': ('promote',  'Gateway Auth — real infra need; small focused proposal'),
    '17735267': ('promote',  'Agent Hooks in VS Code — concrete + actionable for our workflow'),
    '17739741': ('archived', 'Squad AI flow — link only, cost-prohibitive to deeply investigate'),
    '17740194': ('archived', 'Squad vs Superpowers — link only, no analysis hooks'),
    '17742746': ('someday',  'Hormozi/Nate B Jones delegation — captured in art-of-delegation study; revisit if needed'),
    '17743563': ('archived', 'Agentic platform engineering blog link — too generic'),
    '17743715': ('someday',  'Copilot skills examples — our skills/ dir is ahead of these examples; revisit on slow week'),
    '17744822': ('someday',  'Study material digestion — broad; revisit when WS6 wants new tooling'),
    '17746750': ('archived', 'AI skills/career YT — already covered in study/ai/relavent.md'),
    '17750525': ('someday',  'Copilot agent-driven dev blog — keep for reference; not actionable now'),
    '17739252': ('someday',  'Intelligence study YT ref — keep as reference for future study session'),
    '22b8d8b2': ('keep-active', 'Marshfield science center — real life project, keep status=active in Space Center'),
    '17729989': ('keep-active', 'Build Physical Display Dashboard — Space Center project, planned, keep'),
    '20ce036a': ('someday',  'Pull cpuchip.net from wayback — real but not urgent; keep in cpuchip.net project'),
    # ============ RAW — research backlog (Phase C) ============
    '17764487': ('someday',  'Typst for math symbols — useful if we ever do scripture math typesetting'),
    '17756547': ('someday',  'Mempalace memory framework — interesting but not urgent'),
    '17755019': ('someday',  'Continual learning — research direction, defer'),
    '17729953': ('archived', 'Productivity stack — vague; covered by current setup'),
    ...
    '17752798': ('archived', 'AI Dungeons YT — link only, no context'),
    '17752224': ('archived', 'Trinity 57B model — cost-prohibitive to trial'),
    '17751941': ('someday',  'LightRAG continued research — mentioned twice; consolidate with #17751939'),
    '17751939': ('promote',  'LightRAG investigate — promote as research-spike proposal in WS3'),
    '17750573': ('archived', 'HIPAA-compliant AI for Bryce PT — out of scope for our workspace'),
    '17746749': ('someday',  'Long-running Claude research — interesting, revisit when planning long agents'),
    '17750225': ('someday',  'MetaClaw memory framework — defer to memory architecture work'),
    '17743562': ('archived', 'CALM on Linux/vLLM — Linux-side experiment, low priority'),
    'a4eae47c': ('archived', 'Agentic AI memory site eval — duplicate of 10839464'),
    '10839464': ('archived', 'Same site eval — vague, no action'),
    ...
    '17763075': ('someday',  'Personal vs work brains — interesting org idea, defer'),
    '17763948': ('someday',  'Better file format for agents — research, defer'),
    '17765046': ('someday',  'unknown — review body'),
    '17765308': ('someday',  'unknown — review body'),
    '17766038': ('someday',  'GPU requirements for training small model — research, defer'),
    '17765322': ('someday',  'unknown — review body'),
    '17765330': ('someday',  'unknown — review body'),
    '17765321': ('someday',  'unknown — review body'),
    '17765341': ('someday',  'unknown — review body'),
    '17767299': ('someday',  'unknown — review body'),
    '17767123': ('someday',  'unknown — review body'),
    '17767117': ('someday',  'unknown — review body'),
    '17765206': ('someday',  'unknown — review body'),
    '17765205': ('someday',  'unknown — review body'),
    '17767252': ('someday',  'unknown — review body'),
    '17767253': ('someday',  'unknown — review body'),
    '17765208': ('someday',  'unknown — review body'),
    '17767308': ('someday',  'unknown — review body'),
}
...
SELECT e.id, COALESCE(p.name,'<inbox>') AS proj, e.workstream, e.maturity,
       COALESCE(e.status,'<null>') AS status, e.title,
       COALESCE(e.one_liner, e.body) AS body, e.proposal_path
FROM entries e LEFT JOIN projects p ON p.id=e.project_id
WHERE (e.status IS NULL OR e.status IN ('active','someday','roadmap','waiting'))
  AND (e.action_done = 0 OR e.action_done IS NULL)
  AND e.proposal_path IS NULL
...
print(f"# Brain Cleanup Triage — {len(rows)} entries\n")
print("**Generated:** 2026-04-22\n")
print("## How to use this file\n")
print("Review each entry below. The `Recommendation` line is my suggested action with reason.\n")
print("If you agree, leave `Action:` blank (I'll use the recommendation).")
print("If you disagree, fill in `Action:` with one of:\n")
print("- `done` — work is shipped, mark status=done")
print("- `archived` — not pursuing, mark status=archived")
print("- `someday` — interesting but deferred, mark status=someday")
print("- `promote` — worth a workspace proposal, I'll create a one-liner spec")
print("- `keep-active` — leave alone (already in active state)")
print("- `move-to-personal` — move to Notes/Personal project (id=11)")
print("- `merge:<8charID>` — duplicate of another entry, merge into it")
print()
print("Optional `Reason:` line for any override. Hand back when done.\n")

counts = {}
for r in rows:
    rec, _ = RECS.get(r['id'][:8], DEFAULT)
    counts[rec] = counts.get(rec, 0) + 1
print(f"**Recommendation totals:** " + ", ".join(f"`{k}`={v}" for k, v in sorted(counts.items())))
print()
...
for mat in ['verified', 'planned', 'specced', 'raw']:
    ...
    for e in entries:
        ...
        print(f"- **Project:** {e['proj']} · **WS:** {e['workstream'] or '?'} · **Status:** {e['status']}")
        if body and body.lower() != e['title'].lower():
            print(f"- **Body:** {body}")
        print(f"- **Recommendation:** `{rec_action}` — {rec_reason}")
        print(f"- **Action:** ")
        print(f"- **Reason:** ")
        print()

c.close()
"""Generate triage file for remaining unprocessed entries."""
import sqlite3
c = sqlite3.connect('private-brain/brain.db')
c.row_factory = sqlite3.Row
cur = c.cursor()

# Everything that needs a verdict:
# - status NULL or someday/roadmap/active (not done/archived)
# - not in Personal project (id=11)
# - not action_done=1 (already closed by action)
# - not already linked to a proposal (proposal_path NULL)
# Unless it's a "verified" entry without a proposal (those are inspiration candidates - Phase D)
rows = cur.execute("""
SELECT e.id, COALESCE(p.name,'<inbox>') AS proj, e.workstream, e.maturity,
       COALESCE(e.status,'<null>') AS status, e.title,
       COALESCE(e.one_liner, e.body) AS body, e.proposal_path,
...
print(f"# Brain Cleanup Triage — {len(rows)} entries\n")
print("**Instructions:** For each entry below, fill in the `Action:` and `Reason:` lines.")
print("Valid actions: `done`, `archived`, `someday`, `promote` (= create a proposal), `keep-active`, `move-to-personal`, `merge:<entry_id>`, `skip`.")
print()

# Group by maturity for natural triage flow
by_maturity = {}
for r in rows:
    by_maturity.setdefault(r['maturity'], []).append(r)

order = ['verified', 'planned', 'specced', 'raw']
for mat in order:
    entries = by_maturity.get(mat, [])
    if not entries:
        continue
    ...
    for e in entries:
        ...
        if body and body.lower() != e['title'].lower():
            print(f"- **Body:** {body}")
        # My recommendation will be filled in by hand below
        print(f"- **Recommendation:** _(see notes)_")
        print(f"- **Action:** ")
        print(f"- **Reason:** ")
        print()
