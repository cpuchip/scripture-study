...
**Status:** ✅ **v1 COMPLETE + LIVE IN BOTH SPACES (2026-06-02).** **Source of truth is the repo** — `projects/strongs-concordance-mcp/` (`github.com/cpuchip/strongs-concordance-mcp`), see its `README.md` + `docs/data-formats.md` + `data/ATTRIBUTION.md`. **All phases P0–P5 shipped:** dual lexicon (`strongs_define`/`strongs_search`, 19,570 entries) + KJV word-by-word tagging (`strongs_for_verse`, 31,102 verses / 66 books, validation-gated). Registered in **both consumers** — Claude Code (`.mcp.json`) and pg-ai-stewards (bridge image + `stewards.mcp_servers` seed `strongs1-mcp-seed.sql` + study/lesson/talk grants, live-verified via `compose_tools`). Ratified options: dual lexicon (Strong's 1890 + STEPBible) · `for_verse` in v1 via validated KJV tagging · full build grant ([[feedback_strongs_mcp_stewardship]]). This proposal is kept as the origin record; do not duplicate the repo's spec here. Build/deploy detail in the 2026-06-02 v1-complete banner in `.mind/active.md` + the journal entry. Planned v2+: `strongs_occurrences`.
