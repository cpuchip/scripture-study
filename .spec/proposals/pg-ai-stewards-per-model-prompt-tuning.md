...
2. Bigger surface gains exist upstream of voice. **3e (MCP server)** unlocks the substrate as an external tool surface for IDE agents. **3c.4 (gospel-engine HTTP tools)** unlocks real scripture-quote verification at agent runtime. **3f (web UI)** unlocks the reading surface. All three move the substrate from "internally functional" to "externally useful" — voice tuning is downstream of them.
...
After 3e (MCP), 3f (web UI), and 3g (multi-provider expansion). Tentatively phased as:
