...
3. **Model experiments** — PASS 1 COMPLETE (Mar 28). All 5 models run through full 12-test suite (6 prompts × 2 content files). Results at `experiments/lm-studio/scripts/results/` (84 JSON files) and `results.tsv`. Speed ranking: nemotron (85 tok/s) > lfm2 (74) > glm (50) ≈ qwen (49) > devstral (36). Quality: GLM most nuanced TITSW scorer, nemotron caught teach_about_christ=1 on Alma 32 (correct). All 5 passed needle tests. Discovery: qwen3.5 and glm have thinking modes that produce empty responses — fixed with `-NoThink` flag. Nemotron/lfm2 output truncated at default 2048 max_tokens. **Sweet spot: nemotron-3-nano** (fastest, good quality, 1M context, no thinking trap). Full analysis: [.spec/scratch/lm-studio-model-experiments/main.md](../scratch/lm-studio-model-experiments/main.md) → "Pass 1 Results." **Next: Pass 2 — re-run nemotron/lfm2 at 4096 max_tokens, prompt tuning, design conference reindex prompt.**
