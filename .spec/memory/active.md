...
3. **Model experiments** — PASSES 1-2 COMPLETE. TITSW V2 VALIDATED. DEEP STUDY DONE. Nemotron-3-nano confirmed as batch winner (3x faster than GLM, 18.5s avg per talk). **CONTEXT ENGINEERING: PROPOSAL UPDATED, SPLIT INTO TWO STREAMS.** API caching research complete — `cache_prompt: true` eliminates ~44M tokens of redundant system prompt prefill across 5,500-talk batch. Dev handoff spec: [.spec/proposals/context-engineering-dev.md](../proposals/context-engineering-dev.md). **STUDY STREAM COMPLETE.** Both context documents delivered:
   ...
   **ENRICHED INDEXER: PROPOSAL WRITTEN (Mar 29).** Debug audit identified the gap: gospel-vec has 3 generic prompts, zero TITSW vocabulary, produces thin metadata. Lens vs. vocabulary distinction confirmed: scripture needs context docs (lens), talks need TITSW terms in system prompt (vocabulary). Proposal at [.spec/proposals/enriched-indexer.md](../proposals/enriched-indexer.md). Scratch: [.spec/scratch/enriched-indexer/main.md](../scratch/enriched-indexer/main.md). 4 phases designed. Phase 1 (talk enrichment) is one dev session. **Next:** Michael reviews proposal + decides 5 open questions → dev agent builds Phase 1.
