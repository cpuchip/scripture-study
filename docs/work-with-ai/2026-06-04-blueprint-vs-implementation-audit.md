...
The substrate had only **layer 1** (correctness: `verify` runs build+test). The guide's own Step-7 table names three layers — Correctness / Specification / Intent — and warns that correctness-only review certifies "correct implementation of the wrong thing." That warning came true in the night build. **The critic stage now implements layers 2 and 3** as a pipeline stage: a different strong model checks the real diff against the acceptance criteria (spec) and the binding question (intent), and bounces deficiencies back. **Two lessons for the guide:** (a) the three-layer review is *buildable as a stage*, not just a human discipline; (b) **narrow gates raise the floor; full-context vigilance raises the ceiling** — glm-5.1's latent data race passed build, vet, `-race`, *and* the critic; the full-context shepherd (the orchestrating Opus holding the whole arc) caught it — not a human, not any single gate. Review-against-intent is necessary and was missing; it is still not sufficient — a reviewer holding the *whole picture* is what closes the gap (the axis is full-context vs. narrow-gate, not human-vs-AI). Feeds Part Two ch. 04 (`watched_until_they_obeyed`) directly — this is its lived example.
...
1. **Add the "floor vs. ceiling" law to Step 7.** Automating Specification + Intent review raises the floor (no more silent dropped requirements / vacuous tests) but does not remove the need for a **full-context watcher**: subtle soundness (glm's race) slips every *narrow* gate, and here the orchestrating Opus shepherd — holding the whole arc, not a human and not any single gate — caught it. The axis is **full-context vigilance vs. narrow gate, not human-vs-AI.** The human stays the **Hinge** (merge authority / owns the consequence); a full-context reviewer (human *or* AI) is what holds the ceiling. The guide currently implies review can be fully delegated to gates; it can't.
