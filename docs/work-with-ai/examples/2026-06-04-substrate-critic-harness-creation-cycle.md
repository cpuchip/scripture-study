...
- **The meta-finding worth the most:** glm's data race passed build, vet, the race detector, *and* the critic — every automated gate — because the tests didn't exercise that path. A **human read** caught it. So the critic **raises the floor** (no more silently-dropped requirements or vacuous tests) but the **human-merge stays the ceiling** for subtle soundness. The lesson for the book: review-against-intent is necessary and was missing; it is still not *sufficient*. Step 7 has three layers and a human; automating the middle two does not delete the need for the watcher.
