...
**The oracle is the switch (2026-06-29, ratified).** What separates "act" from "surface" is measured, not guessed: Michael overrides on *intent* (vision, voice, content, doctrine, strategy) and accepts on *execution* (the profile finding, `private/michael-profile/`). So a change is act-and-report when it is **execution** AND a deterministic oracle covers it and is green (`scripts/oracles/run.py`, or the relevant build/test/lint/verify); it is **surface-first** when it touches **intent**, a new standing capability, or anything irreversible/outward — regardless of oracle. Absence of an oracle pushes a borderline call toward surface. The oracle is what makes the "act" side safe; widen autonomy by widening the verification floor, not by trusting harder. Run the in-scope oracles before yielding on prose/code work. Full rationale: `.spec/proposals/oracle-floored-autonomy.md`.
