...
**Start with discovery, not recall.** Before drafting, run at least one `gospel_search` (semantic or hybrid) on the lesson's binding question or central scripture. Per Anthropic's 4.7 guide, this model uses tools less by default — reach for them explicitly. Semantic search surfaces conference talks and cross-references your memory wouldn't.

The rhythm:
1. `gospel_search` (semantic or hybrid) on the lesson's binding question → note paths in scratch file
