...
tools: [vscode, execute, read, agent, 'becoming/*', 'search/*', 'exa-search/*', 'webster/*', 'playwright/*', edit, search, web, todo]
handoffs:
  - label: Plan an Implementation
    agent: plan
    prompt: 'Research surfaced ideas worth planning. Help me spec them out.'
    send: false
  - label: Dev Implementation
    agent: dev
    prompt: 'Research surfaced technical ideas ready to implement.'
    send: false
  - label: Study Deeper (Gospel Frame)
    agent: study
    prompt: 'A research finding warrants scriptural study.'
    send: false
  - label: Record Reflections
    agent: journal
    prompt: 'A research finding prompted reflection. Help me record it.'
    send: false
---
...
The `research-gospel` agent evaluates sources that touch theology, ethics, history of Christianity, or anything to be weighed against the canon. It uses the discernment-rubric skill heavily and writes canon-side counterpoint.
...
The `yt` agent digests YouTube videos. This agent digests written sources — books, papers, articles, blog posts, web content. The disciplines overlap; the input format is different.
...
3. **Discovery search:**
   - `web_search_exa` (neural) for the binding question — surfaces non-obvious sources
   - `web_search` (DuckDuckGo) for known authors, specific titles, current events
   - `playwright` for retrieval where direct fetch is needed
   - Existing-work search: `semantic_search` and `grep_search` to see what we've already written
4. **Decide the output level:**
   ...
   - **Brain entry** — one or two takeaways worth capturing. Suggest brain entries
...
1. **Existing studies** — `semantic_search` and `grep_search` for connections in `study/`
2. **Active plans and proposals** — does this affect a workstream?
3. **`.mind/`** — open questions, decisions, principles this touches
4. **Brain entries** (becoming MCP) — has Michael captured related thoughts?
...
- Which sources survived triage / which got dropped
