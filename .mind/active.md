...
*Updated: 2026-05-04 (pg-ai-stewards Phase 2.7a shipped — Watchman substrate; anti-loop discipline enforced by schema, not agent rules; AGE-QUIRKS.md filed; Phase 6 added for AGE upstream contributions; Phase 2.7 honestly split into 2.7a substrate + 2.7b automation-blocked-on-Phase-3) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
...
5. **[WS5] pg-ai-stewards** — Postgres-as-AI-substrate. **Phases 2.5 + 2.6a/b/c + 2.7a shipped May 4.** Workstream vertices + frontmatter `:DECLARED` edges + todos as persistent connector vertices + phase splitter + `context_for(slug, depth)` graph walk + **Watchman substrate (anti-loop discipline enforced by schema)**. **2.7a:** `stewards.studies.last_consolidated_at` column (reused existing `updated_at` as dirty-bit since touch_study() already only bumps it on semantic changes); `stewards.verdicts` (clean|drift|done|superseded|skipped); `stewards.findings` (drift + synthesis with `acknowledged_at` enforcing surface-once-and-stop, resolution ∈ {acted, dismissed, deferred}); `stewards.dirty_queue` view = `WHERE (updated_at > coalesce(last_consolidated_at, '-infinity')) AND NOT EXISTS (open drift finding)` — the NOT EXISTS clause IS the discipline; brain v1's loop is structurally impossible. Verified end-to-end via Agans Rule 9: edit→drift-verdict→finding→**touch again→stays-out-of-queue**→ack→returns. CLI now: `workstream`, `edges`, `todo`, `context`, `watchman queue|verdict|finding|ack|history`. **AGE-QUIRKS.md filed** at `projects/pg-ai-stewards/docs/AGE-QUIRKS.md` with 8 quirks categorized (3 PR-worthy bug-candidates, 3 spec-divergences, 1 by-design, 1 our-mistake). **Phase 6 added** for AGE upstream contributions (independent of delivery sequencing). **Honest scope split:** Phase 2.7 split into 2.7a (substrate, shipped) and 2.7b (bgworker automation, blocked on Phase 3 model dispatch). **Foldback debt** now FOUR files: 2-6a/b/c + 2-7a SQL → lib.rs at next intentional rebuild. **Phase 3 next real frontier** (model dispatch sidecar; unblocks 2.7b). → [proposal](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) · [2.7a SQL](../projects/pg-ai-stewards/extension/2-7a-watchman-substrate.sql) · [AGE-QUIRKS.md](../projects/pg-ai-stewards/docs/AGE-QUIRKS.md) · [phases.md](../projects/pg-ai-stewards/phases.md)
...
| WS5 | pg-ai-stewards (Postgres substrate for agent state, memory, work, model calls) | 🔨 **Phases 2.5 + 2.6a/b/c + 2.7a shipped May 4.** Workstreams + typed declared edges + todos + phase splitter + `context_for` graph walk + **Watchman substrate** (dirty-bit, verdicts, findings, surface-once-and-stop). Anti-loop discipline enforced by schema (NOT EXISTS in view), not agent rules. CLI: workstream, edges, todo, context, **watchman queue\|verdict\|finding\|ack\|history**. AGE-QUIRKS.md filed (8 quirks categorized: 3 PR-worthy, 3 spec-divergence, 1 by-design, 1 our-mistake). Phase 6 added for AGE upstream PRs. **2.7b blocked on Phase 3 model dispatch.** | [proposal](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) |
