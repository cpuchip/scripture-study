...
*Updated: 2026-05-04 (pg-ai-stewards Phase 2.6c shipped — phase splitter + context_for graph walk; substrate closed its own loops for the first time; AGE quirks 5-8 logged; Phase 5 multi-tenant spec'd; a.ibeco.me named for Phase 3 frontend) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
...
5. **[WS5] pg-ai-stewards** — Postgres-as-AI-substrate. **Phase 2.5 done May 4 — generic document substrate.** **Phases 2.6a + 2.6b + 2.6c shipped May 4** — workstream vertices + frontmatter `:DECLARED` edges + todos as persistent connector vertices + **phase splitter + `context_for(slug, depth)` graph walk**. WS1–WS9 seeded; `link_declared_edges()` parses `workstream`/`feeds`/`supersedes`/`implements`. `stewards.todos` table + `:HAS_TODO` edges; single-write `create_todo()` / `complete_todo()` functions. **`## Phase X.Y` headers in `phases.md` now produce per-phase :Study rows with kind=phase, linked back to phase-doc via :HAS_PHASE.** **`stewards.context_for(slug, depth)` returns shaped graph briefings** — e.g. WS5 at depth=1 returns 9 :HAS_PROPOSAL + 3 :HAS_TODO; the proposal at depth=1 returns 3 :FEEDS + 1 :HAS_TODO + inbound :HAS_PROPOSAL + 8 :SIMILAR_TO. CLI: `workstream`, `edges`, `todo create|done|list|audit`, `context <slug> [--depth N]`. **Dogfood moment: 2.6b''s todos closed by 2.6c via the same CLI.** AGE quirks list at 8. **Phase 5 multi-tenant** spec'd in detail (workstream as unit of sharing, AGE-Cypher-doesn''t-auto-enforce-RLS caveat, three mitigation paths, user_secrets vault). **Phase 3 reading surface** named `a.ibeco.me` ("a becoming" / "ai become"). **Foldback debt**: 2-6a/b/c SQL needs to fold into lib.rs at next intentional rebuild. **Phase 2.7 next** (Watchman + AGE-QUIRKS.md). → [scripts CLI README](../projects/pg-ai-stewards/cmd/stewards-cli/README.md) · [proposal](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) · [2.6c SQL](../projects/pg-ai-stewards/extension/2-6c-phases-context.sql) · [phases.md](../projects/pg-ai-stewards/phases.md)
...
| WS5 | pg-ai-stewards (Postgres substrate for agent state, memory, work, model calls) | 🔨 **Phases 2.5 + 2.6a/b/c shipped May 4.** Workstreams + typed declared edges + todos + phase splitter + `context_for` graph walk. CLI: workstream, edges, todo, context. **Dogfood moment** — 2.6b''s todos closed by 2.6c. Phase 5 multi-tenant spec'd. **2.7 next** (Watchman + AGE-QUIRKS.md). | [proposal](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) |
