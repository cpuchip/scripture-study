...
*Updated: 2026-05-06 (pg-ai-stewards Phase 2.7 fully shipped — code-side. **2.7b.1/b.2/b.3/b.4** all in place. 2.7b.4 added the `dirty_queue` exemption for `frontmatter:watchman:skip` AND `regenerate_active_md()` (returns markdown status report; CLI `watchman active-md`). docs/architecture.md written — 466-line reading map of the substrate. Closed todo `watchman-frontmatter-exempt`. **Soak hasn't started** — first need to bulk-tag the 70 journal YAML files with `watchman: skip` then re-import (otherwise journals generate noise). Nine `extension_sql_file!` foldback ready; image rebuild deferred to next batched change. Container at v0.2.0 with all live-applied. **Next: agent corpus import (option A from the conversation) — Copilot/Claude agent definitions into `stewards.agents` + skills into `stewards.skills`.**) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
...
| WS5 | pg-ai-stewards (Postgres substrate for agent state, memory, work, model calls) | 🔨 **Phase 2.7 fully shipped (b.1/b.2/b.3/b.4) May 4–6.** Plus **docs/architecture.md** as the reading map. **Next: bulk-tag journals + start the soak; in parallel, import the agent corpus (option A) into `stewards.agents`/`stewards.skills`.** | [proposal](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) |
