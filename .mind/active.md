...
> **2026-05-20 — 1828-illuminated phase 5 (frontend cutover) SHIPPED same day as phases 1-4.** WS7. Six commits on `main` (6b0e98a + 36419b1 + 70ede1f + 554db8c + bbc1466 + 07bf9f3). The static-JSON-import Vue SPA is gone — useWordData reaches `/api/dict/{1828,modern,search}` async with a 200-entry LRU; useLLMSettings + useLLMSession + useLLMRender carry the BYOK + session-key flow with the api_key never persisted client-side (v1→v2 localStorage migration banner on first load). VerseExplorer gained "Browse canon" mode driving `/api/scripture/chapter/{ref}` with the existing tier-word HighlightedText highlighting + churchofjesuschrist.org breakout. WordSearch surfaces class-E reach below the curated section (any of the 98,828 1828 headwords reachable, not just the 853 tier-curated). Settings.vue rebuilt around session lifecycle: Start session probes the key server-side, transient api-key input never bound to llmSettings, Sign out drops the held key on the backend. WordCard + Present.vue moved to async pattern with stem_matched surfaced ("showing entry for `obtain`" when reader clicked "obtaineth"). useWordData chunk dropped from ~1.5MB (static dict bundles) to 247KB. Final smoke: all containers healthy, legacy 8082 untouched, full session lifecycle (mint→render→inspect-reject) verified via mock provider. Phases 6 (Thummim sync) + 7 (production deploy) deferred. Substrate untouched.
