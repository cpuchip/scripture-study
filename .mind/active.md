...
> **2026-05-20 — 1828-illuminated backend pivot phases 1–4 SHIPPED.** WS7. Four commits on `main` (6b0e98a + 36419b1 + 70ede1f + 554db8c). Three-container stack at `projects/1828-illuminated/`: nginx frontend on host port 8083 (legacy `1828` container untouched on 8082), Go backend (distroless static) on internal 8080, Postgres 17-alpine on its own `i1828-pg-data` volume. 11 tables, 41,995 verses + 98,828 1828 headwords + 859 tier words + 710 modern defs ingested at first boot. Every endpoint smoke-tested: scripture get/chapter/search/word-study, dict 1828/modern/tier/search (modern lazy-fetched + write-back from Free Dictionary API confirmed), LLM session mint/inspect/render/delete with full BYOK round-trip + per-IP rate-limit body shape verified per D-BE-AUTH. Phase 5 (frontend cutover) = next session — useWordData.ts / useLLMRender.ts / useLLMSettings.ts swap to API; carry-forward detailed in phase 4 commit body. Phases 6+7 deferred. Substrate untouched.
...
| WS7 | 1828-illuminated scriptures (1828.ibeco.me) | 🔨 **Backend phases 1–4 SHIPPED 2026-05-20.** Three-container stack stands up healthy from a fresh checkout: nginx-frontend on host port 8083 (avoiding legacy 1828 container on 8082) + Go backend (distroless static) + Postgres 17-alpine with 11 tables. 41,995 verses + 98,828 1828 headwords + 859 tier words + 710 modern defs ingested. All endpoints smoke-tested: `/api/scripture/{ref,chapter,search,word-study}`, `/api/dict/{1828,modern,tier,search}`, `/api/llm/{session,render}` with full BYOK round-trip + rate_limited_by_1828 body shape verified per D-BE-AUTH. Steward-mode fixes folded in: `.dockerignore .env` exclusion, scripture space-before-punct after bracket-strip, nginx keepalive, frontend healthcheck IPv6→IPv4, session-id log truncation, polite User-Agent on Free Dictionary fetcher. Commits 6b0e98a + 36419b1 + 70ede1f + 554db8c. Phase 5 (frontend cutover) is the next session's work; phases 6+7 (Thummim sync + backup polish) deferred. | [journal](../projects/1828-illuminated/.spec/journal/2026-05-20-backend-phase-1234-build.md) · [proposals](../projects/1828-illuminated/.spec/proposals/) · [project CLAUDE.md](../projects/1828-illuminated/CLAUDE.md) |
