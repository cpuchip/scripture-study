...
*Updated: 2026-05-05 (pg-ai-stewards Phase 3a shipped — model dispatch via opencode_go + kimi-k2.6 unblocks Watchman; CLI `stewards-cli watchman pass` produces real model verdicts, recorded in stewards.verdicts with actor=watchman + model + tokens; substrate consolidates its own docs end-to-end; Phase 3 honestly split into 3a-3g) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
...
5. **[WS5] pg-ai-stewards** — Postgres-as-AI-substrate. **Phases 2.5 + 2.6a/b/c + 2.7a + 3a shipped May 4-5.** **3a:** model dispatch end-to-end. `watchman-consolidator` agent (no tools, temp=0, steps=1) + `stewards.watchman_input(slug)` (doc body + 1-hop graph) + Go orchestrator `WatchmanPass` (dirty_queue → chat_enqueue → poll → parse JSON → record_verdict + record_finding). Bgworker stays generic; watchman semantics live in CLI. Verified end-to-end with kimi-k2.6 via opencode_go: 2 verdicts in `stewards.verdicts` with actor=watchman, model+tokens logged. The first verdict (`skipped` on phase-pg-ai-stewards-0) demonstrated the discipline — kimi admitted what it couldn't see (linked files outside the 1-hop neighborhood) AND surfaced a 3b agenda item (include linked files in review neighborhood). Bgworker 120s reqwest ceiling is the real limit on big docs (proposal + 2 large scratch files timed out); Phase 3b candidates: trim input OR bump timeout. **Honest scope split:** Phase 3 split into 3a (shipped) + 3b-3g (response_format injection, pipelines, sidecars, MCP, web UI, multi-provider). 3a is what unblocks 2.7b/2.8/`a.ibeco.me`. **Foldback debt** now FIVE files: 2-6a/b/c + 2-7a + 3a SQL → lib.rs at next intentional rebuild. → [proposal](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) · [3a SQL](../projects/pg-ai-stewards/extension/3a-watchman-pass.sql) · [phase-3a journal](../.spec/journal/2026-05-05-pg-ai-stewards-phase-3a.md) · [phases.md](../projects/pg-ai-stewards/phases.md)
...
| WS5 | pg-ai-stewards (Postgres substrate for agent state, memory, work, model calls) | 🔨 **Phases 2.5 + 2.6a/b/c + 2.7a + 3a shipped May 4-5.** Workstreams + typed declared edges + todos + phase splitter + `context_for` graph walk + Watchman substrate + **model dispatch (kimi-k2.6 via opencode_go)**. CLI: workstream, edges, todo, context, watchman queue\|verdict\|finding\|ack\|history\|**pass**. AGE-QUIRKS.md filed (8 quirks). Phase 6 added for AGE upstream PRs. **3a unblocks 2.7b automation; 3b-3g remain.** | [proposal](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) |
