...
*Updated: 2026-05-03 (pg-ai-stewards Phase 1.6 done — full agent loop verified end-to-end on kimi-k2.6, success path + inverse hypothesis both green) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
...
5. **[WS5] pg-ai-stewards** — Postgres-as-AI-substrate. **Phase 1.6 done May 3 — full agent loop runs end-to-end.** kimi-k2.6 takes a question, invokes tools, reads tool replies, continues, terminates with `finish_reason='stop'`. Inverse hypothesis green: a tool that errors lands as `role='tool'` content; the model reads the error and recovers. Both `reasoning_content` and `reasoning_details` captured + echoed back (Moonshot 400s otherwise). Loop architecture is work-item-per-iteration (each chat turn + each tool dispatch is its own `work_queue` row), so every step is observable, cancellable, and the bgworker can't be starved by long tool calls. Two non-obvious quirks named in `/memories/repo/pg-ai-stewards.md`: (1) PgTryBuilder doesn't catch ereports through `BackgroundWorker::transaction` in pgrx 0.18 — workaround is pre-flight `pg_proc` lookup; (2) SAVEPOINT can't be issued inside the implicit bgworker tx. Stale-claim reaper at startup catches any future bgworker crashes. Next: prompt caching strategy verification (every loop body should keep an identical system+tools prefix), exposing the loop to a higher-level chat API. → [projects/pg-ai-stewards/extension/README.md](../projects/pg-ai-stewards/extension/README.md) · [phases.md](../projects/pg-ai-stewards/phases.md)
...
| WS5 | pg-ai-stewards (Postgres substrate for agent state, memory, work, model calls) | 🔨 **Phase 1.6 done May 3** — full agent loop end-to-end on kimi-k2.6. Tool calls + continuation + reasoning replay + error recovery all verified. Next: prompt caching audit, higher-level chat API. | [projects/pg-ai-stewards/extension/README.md](../projects/pg-ai-stewards/extension/README.md) |
