...
*Updated: 2026-05-04 (pg-ai-stewards Phase 2.4 done — `stewards study show` CLI ties together studies + resolved citations + similar studies; Phase 2 done criteria met; generalization-to-other-corpora question surfaced for Phase 2.5) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
...
5. **[WS5] pg-ai-stewards** — Postgres-as-AI-substrate. **Phase 2.4 done May 4 — `stewards study show` CLI shipped; Phase 2 done criteria met.** `stewards.study_show(slug)` SQL fn + thin PowerShell wrapper [stewards.ps1](../projects/pg-ai-stewards/extension/stewards.ps1) (`study show | list | refresh`). Running it on `give-away-all-my-sins` returns the study + 14 resolved scripture verses across 6 citations + 5 similar studies (`atoning-love-andersen` 0.815, `only-begotten` 0.803, `moses-6-gospel-to-adam`, `know-god`, `receive`). Real corpus signal: studies titled "Charity", "Agency", "Creation Patterns: Divine Design and AI Collaboration" already do the bridge between gospel and AI principles — prompts the Phase 2.5 question of importing `docs/work-with-ai/*-gospel.md` into the same graph (importer hardcoded to `study/` but schema is generic; recommendation: add `corpus` column, parameterize sources, keep `:Study` AGE label). Phase 2.3 (May 4): similarity bridge, 345 edges. 2.2: resolver, 87.2% verse coverage. 2.1: 69 studies imported. 1.6: full agent loop. Next: 2.5 corpus generalization, OR pivot to Phase 3 (pipelines + MCP). → [projects/pg-ai-stewards/extension/README.md](../projects/pg-ai-stewards/extension/README.md) · [phases.md](../projects/pg-ai-stewards/phases.md)
...
| WS5 | pg-ai-stewards (Postgres substrate for agent state, memory, work, model calls) | 🔨 **Phase 2.4 done May 4 — `stewards study show` CLI; Phase 2 done.** Single SQL fn formats study + resolved citations + similar studies; PowerShell wrapper at `extension/stewards.ps1`. Generalization question: import `docs/work-with-ai/` into the same graph? → Phase 2.5 candidate. | [projects/pg-ai-stewards/extension/README.md](../projects/pg-ai-stewards/extension/README.md) |
