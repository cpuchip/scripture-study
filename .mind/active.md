...
*Updated: 2026-05-07 (pg-ai-stewards Phase 2.7 + 3a.1 + journal-normalization + 3c.1 shipped May 6–7. **Phase 3c.1 (pipelines + work_items)** ships the orchestration layer above `work_queue`: pipeline definitions as immutable templates, work_items as instances flowing through stages, transition functions (`work_item_create/dispatch_stage/advance/fail/cancel`), seed `echo-test` pipeline, full CLI surface. End-to-end smoke test verified: dispatch enqueues chat with payload markers (`_work_item_id` / `_stage_name` / `_pipeline_family`), bgworker dispatches normally (kimi-k2.6 returned "ack" 1935/30 tokens), manual advance records output in `stage_results` and marks completed. **Same architecture as 2.7b.1** — orchestration semantics in SQL, dispatch generic in Rust. Auto-advance trigger is 3c.2 (next session). Three unpushed commits before this one (Michael inspecting). Soak still not flipped on. **Next: 3c.2 (auto-advance trigger), then 3c.3 (first real multi-stage pipeline using imported agent corpus).**) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
...
| WS5 | pg-ai-stewards (Postgres substrate for agent state, memory, work, model calls) | 🔨 **Phase 2.7 + 3a.1 + 3c.1 shipped May 6–7.** Watchman stack + agent corpus + pipelines/work_items orchestration layer. End-to-end pipeline flow verified (echo-test). **Next: 3c.2 (auto-advance trigger), then 3c.3 (first multi-stage pipeline). Soak ready but not started.** | [proposal](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) |
