...
| WS7 | 1828-illuminated scriptures (1828.ibeco.me) | 🔨 **Backend build IN FLIGHT 2026-05-20 (dev agent spawned).** Planning cycle CLOSED with five proposals + 32 ratified decisions + CLAUDE.md/intent.yaml rewrites + .env.example. Backend pivot canonical at `projects/1828-illuminated/.spec/proposals/`. Headline ratifications: **D-BE-COPYRIGHT option D hybrid**; **D-LP-1** opencode-go/zen primary (LM Studio = embeddings only); **D-LP-2** BYOK + 24h sliding-TTL in-memory session key (keys never persisted); **D-SC-2** english tsv + archaic-suffix expansion; **D-DICT-1** full 98k 1828 ingest. Thummim batch closed: 14 verified at $5.00; remaining 7 cancelled (wedged in gather ~14h, no failure reason — bridge worker likely crashed). Markdown entries at `research/dictionary/thummim-*.md`; `stewards.thummim_entries` table empty (D-THM-7 hook deferred — backfill needed). Commits ac687b9 + 3997fbc + ff6ecad + (journal + env). | [proposals](../projects/1828-illuminated/.spec/proposals/) · [journal](../.spec/journal/2026-05-20-1828-backend-pivot-ratified.md) · [project CLAUDE.md](../projects/1828-illuminated/CLAUDE.md) |
