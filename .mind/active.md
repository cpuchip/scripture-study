...
*Updated: 2026-05-04 (pg-ai-stewards Phase 2.6b shipped — todos as connector vertices, :HAS_TODO edges, single-write rule, DB-centric principle named in phases.md Phase 3) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
...
5. **[WS5] pg-ai-stewards** — Postgres-as-AI-substrate. **Phase 2.5 done May 4 — generic document substrate.** **Phase 2.6a + 2.6b shipped May 4** — workstream vertices, frontmatter `:DECLARED` edges, AND todos as persistent connector vertices. WS1–WS9 seeded; `link_declared_edges()` parses `workstream`/`feeds`/`supersedes`/`implements`. `stewards.todos` table + `:HAS_TODO` edges; single-write `create_todo()` / `complete_todo()` functions own both row and graph mutation. CLI: `workstream list|show`, `edges <slug>`, `todo create|done|list|audit`. Three-door schema (declared/linked/inferred) live in production. **First real :Todo rows now exist** — 3 for Phase 2.6c work + 1 fold-back-debt todo on the proposal. **Architectural principle named** (in phases.md Phase 3): pg-ai-stewards is **DB-centric**. Studies/scratch/notes live as DB rows, not files — every output is immediately a graph citizen. File writing is reserved for code + binary deliverables. Reading surface = becoming/ibeco.me. **Phase 2.6c next** (phase splitter + `context_for(slug, depth)` + `regenerate_active_md()`). **Phase 2.7 sketched** (Watchman, 7-day soak). → [scripts CLI README](../projects/pg-ai-stewards/cmd/stewards-cli/README.md) · [proposal (2.5/2.6/2.7)](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) · [2.6a SQL](../projects/pg-ai-stewards/extension/2-6a-workstreams.sql) · [2.6b SQL](../projects/pg-ai-stewards/extension/2-6b-todos.sql) · [phases.md](../projects/pg-ai-stewards/phases.md)
...
| WS5 | pg-ai-stewards (Postgres substrate for agent state, memory, work, model calls) | 🔨 **Phase 2.5 + 2.6a + 2.6b shipped May 4.** WS1–WS9 vertices + frontmatter typed edges + todos with :HAS_TODO. CLI: workstream, edges, todo (create/done/list/audit). DB-centric principle named in Phase 3. **2.6c next** (phase splitter + context_for + regenerate active.md). 2.7 sketched (Watchman, 7-day soak). | [proposal (2.5/2.6/2.7)](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) |
