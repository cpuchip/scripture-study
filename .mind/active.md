...
*Updated: 2026-05-04 (pg-ai-stewards Phase 2.3 done — similarity bridge live; 345 :SIMILAR_TO edges across 69 studies, real semantic clustering verified) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
...
5. **[WS5] pg-ai-stewards** — Postgres-as-AI-substrate. **Phase 2.3 done May 4 — similarity bridge live.** `stewards.refresh_study_similarity(slug, top_k, min_score)` walks pgvector's `<=>` for top-K and writes `:SIMILAR_TO {method:'pgvector_cosine', score}` edges in AGE; `stewards.study_similar(slug)` reads them back labeled outgoing/incoming/mutual. Corpus state: 69 studies × K=5 = 345 edges, ~1s refresh, score range 0.62–0.94. Real semantic clusters: `art-of-delegation` ↔ `stewardship-pattern` (0.888); `charity` ↔ `enoch-charity` (0.843); `give-away-all-my-sins` ↔ `atoning-love-andersen` (0.815). Bug caught by inverse hypothesis: early-return when embedding is NULL skipped the DELETE — stale edges survived “refresh.” Fix: always delete, conditionally skip writes. Phase 2.2 (May 4 earlier): gospel-engine resolver, 87.2% success on full corpus. Phase 2.1: 69 studies imported, 432 scripture vertices, 1256 CITES edges. Phase 1.6: full agent loop on kimi-k2.6. Next: 2.4 `stewards study show` CLI tying study + resolved citations + similar studies into one view. → [projects/pg-ai-stewards/extension/README.md](../projects/pg-ai-stewards/extension/README.md) · [phases.md](../projects/pg-ai-stewards/phases.md)
...
| WS5 | pg-ai-stewards (Postgres substrate for agent state, memory, work, model calls) | 🔨 **Phase 2.3 done May 4** — similarity bridge. `refresh_study_similarity()` writes `:SIMILAR_TO` edges; `study_similar()` reads them with outgoing/incoming/mutual labels. 345 edges across 69 studies, semantic clustering verified. Next: Phase 2.4 CLI. | [projects/pg-ai-stewards/extension/README.md](../projects/pg-ai-stewards/extension/README.md) |
