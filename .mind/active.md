...
5. **[WS5] pg-ai-stewards** — Postgres-as-AI-substrate. **Phase 1 steps 1–3+6+7 done May 2–3; Phase 1.5 (harness sketch) done May 3.** Real LM Studio embeddings landing in vector(768); `dry_run_chat()` returns full OpenAI POST body with variant-by-glob agent/skill/instruction resolution; `chat_enqueue()` composes + persists + enqueues, bgworker POSTs and writes assistant message back (4.4s end-to-end to kimi-k2.6 via OpenCode Go, persona arrived intact). Reasoning tokens captured separately from completion tokens (kimi bills them apart — under-counts cost ~50% if missed); `user: <session_id>` injected into outgoing body for OpenCode's per-session billing dashboard. Confirmed kimi invokes tools when warranted ("name two virtues from Moroni 7" → valid `brain_search_text` call, args `{query: "Moroni 7 virtues faith hope charity", limit: 10}`). Inverse hypothesis green both ways. Caught and removed a `chat_round_trip()` footgun (SQL fn polled in own tx → MVCC blindness + row lock). Next: **Phase 1.6** — tool execution loop. → [projects/pg-ai-stewards/extension/README.md](../projects/pg-ai-stewards/extension/README.md) · [phases.md](../projects/pg-ai-stewards/phases.md)
