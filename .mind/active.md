...
*Updated: 2026-05-04 (pg-ai-stewards Phase 2.2 done — gospel-engine resolver caches verse text for citations; 87.2% success on full corpus, remaining gap is gospel-engine v2 ingest, not stewards) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
...
5. **[WS5] pg-ai-stewards** — Postgres-as-AI-substrate. **Phase 2.2 done May 4 — gospel-engine resolver live.** Citations from imported studies now resolve to actual verse text via cached HTTP round-trips to engine.ibeco.me. New `stewards.resolved_refs` cache (single-verse PK, sticky errors), `parse_reference()` + `normalize_book()` (handles ranges, en-dashes, "Rom."→"Romans" abbreviations, "Psalm"→"Psalms" plural slip), `resolve_ref` work_kind on the bgworker, `study_citations_resolved(slug)` view. Full corpus verified: **1363 unique verses cached, 87.2% success, 0 retries on cached rows.** The 175 misses are real corpus gaps in gospel-engine v2 (Hebrews, James, 1 Cor, Jeremiah, Ezekiel, Job, Proverbs, most general epistles — confirmed by direct curl) — finding to feed back to gospel-engine owner, not a stewards bug. Phase 2.1 (May 4 earlier): 69 studies imported, 432 scripture vertices, 1256 CITES edges in AGE graph. Phase 1.6 (May 3): full agent loop on kimi-k2.6. Next: 2.3 similarity bridge (pgvector → SIMILAR_TO edges), 2.4 `stewards study show` CLI. → [projects/pg-ai-stewards/extension/README.md](../projects/pg-ai-stewards/extension/README.md) · [phases.md](../projects/pg-ai-stewards/phases.md)
...
| WS5 | pg-ai-stewards (Postgres substrate for agent state, memory, work, model calls) | 🔨 **Phase 2.2 done May 4** — gospel-engine resolver. `stewards.resolved_refs` cache + `study_citations_resolved(slug)` view. Full corpus: 1363 verses, 87.2% success. 175 misses are gospel-engine v2 ingest gaps (Hebrews, James, 1 Cor, Jeremiah, etc. — confirmed by direct curl). Next: Phase 2.3 similarity bridge, Phase 2.4 CLI. | [projects/pg-ai-stewards/extension/README.md](../projects/pg-ai-stewards/extension/README.md) |
