...
*Updated: 2026-05-04 (pg-ai-stewards Phase 2.6a shipped — workstream vertices + frontmatter :DECLARED edges; three-door schema live; 2.6b/c next; 2.7 sketched with 7-day soak) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
...
5. **[WS5] pg-ai-stewards** — Postgres-as-AI-substrate. **Phase 2.5 done May 4 — generic document substrate.** **Phase 2.6a shipped May 4** — workstream vertices + frontmatter `:DECLARED` edges. WS1–WS9 seeded from `.mind/workstreams.md`; `link_declared_edges()` parses `workstream` / `feeds` / `supersedes` / `implements` from frontmatter and writes typed edges with `provenance='declared'`, `confidence=1.0`, `source='frontmatter:<key>'`. Verified: WS5 vertex links to all 9 of its declared proposals; this proposal's `feeds:` list produces 3 :FEEDS edges. CLI: `stewards-cli workstream list|show <id>` and `edges <slug>`. Three-door schema (declared/linked/inferred) is now real in production. **Phase 2.6b next** (todos as persistent connector vertices). Phase 2.6c after that (phase splitter + `context_for(slug, depth)`). **Phase 2.7 sketched** (Watchman, brain-v1-anti-loop discipline; soak compressed from 30d to 7d). → [scripts CLI README](../projects/pg-ai-stewards/cmd/stewards-cli/README.md) · [proposal (2.5/2.6/2.7)](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) · [2.6a SQL](../projects/pg-ai-stewards/extension/2-6a-workstreams.sql) · [phases.md](../projects/pg-ai-stewards/phases.md)
...
| WS5 | pg-ai-stewards (Postgres substrate for agent state, memory, work, model calls) | 🔨 **Phase 2.5 + 2.6a shipped May 4.** WS1–WS9 vertices seeded; frontmatter :HAS_PROPOSAL/:FEEDS/:SUPERSEDES/:IMPLEMENTS edges live with provenance='declared'. CLI: workstream list|show, edges <slug>. **Phase 2.6b next** (todo vertices + roll-up audit). 2.6c after (phase splitter + context_for). 2.7 sketched (Watchman, 7-day soak). | [proposal (2.5/2.6/2.7)](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) |
