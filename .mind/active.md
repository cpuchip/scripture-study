...
*Updated: 2026-05-07 (pg-ai-stewards Phase 2.7 + 3a.1 + 3c.1 + 3c.2 shipped; 3c.2.5 spec'd. **3c.2 trigger** auto-advances work_items on chat completion: detects intermediate-vs-final (tool_calls iterations don't advance, only the clean stop or steps_exhausted does), rolls up tokens always, gates auto-dispatch on token_budget (exhausted → status=awaiting_review with explanatory error). Live smoke test: echo-test pipeline auto-completed in ~5s with stage_results populated by trigger; no manual `work-item advance` call. 4-trial synthetic inverse hypothesis confirmed trigger is load-bearing (drop trigger → no advance, no rollup; restore → advances again; budget gate stops dispatch correctly). **3c.2.5 spec'd** with three resolved decisions: line-based pagination on study_get, multi-kind array filter on study_search_text, study_context_for naming. **Next: 3c.2.5 (~30 min in fresh session), then 3c.3 (first real pipeline). Soak still not flipped on.**) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
...
| WS5 | pg-ai-stewards (Postgres substrate for agent state, memory, work, model calls) | 🔨 **Phase 2.7 + 3a.1 + 3c.1 + 3c.2 shipped May 6–7; 3c.2.5 spec'd.** Watchman stack + agent corpus + pipelines/work_items + auto-advance trigger. **Next: 3c.2.5 (study tools, ~30 min), then 3c.3 (first multi-stage pipeline). Soak ready but not started.** | [proposal](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) |
