...
*Updated: 2026-05-07 (pg-ai-stewards 3c.3.1 + 3c.3.2 shipped; **the substrate produced a real meta-study end-to-end.** 3c.3.1 fixed all three bugs from v1: (a) `v_is_final` coalesced to false; (b) `chat_post_internal` now propagates `_*` payload markers from the session's most recent chat (generic across watchman + work_items); (c) `agents.steps` bumped from 8 → 50 for all non-watchman agents. 3c.3.2 re-ran study-write on the same FtC/WtL binding question with budget=2M: 17m14s elapsed, 626K in / 64K out (~$0.30 with caching), 18 chats, 3 stages auto-advanced, **token rollup verified exact** (work_item.tokens matches sum across all session messages). The agent produced a 6-section meta-study with self-review revision notes (em-dash discipline applied; 4 non-verbatim quotes corrected; binding question coverage confirmed). Output saved to `study/.scratch/ftc-wtl-meta-v2-review-output.md`. The synthesis: "These triplets are not the same point seen from different vantage points, yet they are not separate programs either. They are the instrument and the music, the eye and the light, the traveler and the road. The ascent is one, the descriptions are two, and the Person at the threshold is Christ." **The substrate now does meaningful agent work end-to-end.**) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
