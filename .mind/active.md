...
> **2026-05-19 — substrate: Council ① CLOSED — all of PE-A + PE-B + PE-C + end-to-end smoke shipped.** WS5/pg-ai-stewards. 13+ commits in one calendar day, zero rollbacks. PE-A (3 new pipelines + studies/AGE promotion + backfill). PE-B (scheduled_pipelines table + plpgsql cron parser + dispatcher wired into the watchman tick + ai-news-7am seed — no pg rebuild needed; the original "Rust cron crate + 86-file replay" path was reframed to plpgsql). PE-C (full-CRUD `/scheduled` Vue route + Dashboard "Last 7 scheduled runs" card + NewWork.vue per-pipeline forms). PE-final dispatched all three pipelines end-to-end: research-summary (AI news digest, $0.21), yt-secular-digest (Nate B Jones on Pinecone + vector search, $0.08), yt-gospel-evaluate (Morgan Philpot, $0.07). All three completed verified, sabbathed where required, promoted into studies + AGE. One bug surfaced + fixed mid-smoke (yt-* pipelines missing auto_materialize_on_verified=true; fixed in pe3/pe4 sources). Soak resumed. Next: council ② substrate-scheduled-workflows. Canonical navigation: [`projects/pg-ai-stewards/.spec/open-items.md`](../projects/pg-ai-stewards/.spec/open-items.md) §0. Per-session history lives in `.spec/journal/` — this file holds current state, not the archive.
...
| WS5 | pg-ai-stewards (Postgres substrate for agent state, memory, work, model calls) | ✅ **Council ① CLOSED 2026-05-19** (PE-A + PE-B + PE-C + end-to-end smoke of all three pipelines). Next: ② scheduled-workflows → ③ ui-evolution. Direction: hybrid work-together modes. | [open-items.md](../projects/pg-ai-stewards/.spec/open-items.md) §0 |
