...
*Updated: 2026-05-06 (pg-ai-stewards Phase 2.7b.1 + 2.7b.2 + 2.7b.3 shipped same day. **2.7b.1:** trigger-driven Watchman; `AFTER UPDATE` trigger on `work_queue` harvests verdicts/findings transactionally. **2.7b.2:** bgworker scheduler tick (60s); SQL decides pressure/cron/idle. **2.7b.3:** per-pass token budget enforcement; `estimate_chat_tokens(slug)` from input length + system overhead + recent avg output; loop in `watchman_pass_start` stops when next-doc estimate would cross budget; `budget_stopped` flag distinguishes "budget hit" from "queue empty"; verified across 4 budget tiers (0/10k/25k/∞) with no token spend. CLI surfaces budget status in `passes` and `pass-detail`. Eight `extension_sql_file!` foldback (2-7b3 not yet rebuilt into image — pending). Container at v0.2.0; 2-7b3 live-applied. 2.7b.4 (7-day soak + regenerate_active_md) next.) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
...
5. **[WS5] pg-ai-stewards** — Postgres-as-AI-substrate. **Phases 2.5 + 2.6a/b/c + 2.7a + 3a + 3b + 2.7b.1 + 2.7b.2 + 2.7b.3 shipped May 4–6.** **v0.2.0:** eight SQL files via `extension_sql_file!` (2-6a/b/c, 2-7a, 3a, 2-7b1, 2-7b2, 2-7b3). **2.7b.1:** trigger-driven harvest. **2.7b.2:** bgworker scheduler tick (60s); SQL decides pressure/cron/idle. **2.7b.3:** per-pass token budget; `estimate_chat_tokens(slug)` = input/4 + 1500 system overhead + 30d avg output; `watchman_pass_start` exits loop when next-doc estimate would cross budget; `budget_stopped` flag. 4 SQL trials verified at 1k/10k/25k/∞ budgets, zero model tokens spent. CLI surfaces budget status. Container has 2-7b3 live-applied; image rebuild deferred to next batched change (lib.rs + Dockerfile foldback already in place). **Open:** 2.7b.4 (7-day soak + regenerate_active_md), ws6 AGE upstream PRs, 3c–3g. → [proposal](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) · [phases.md](../projects/pg-ai-stewards/phases.md) · [AGE-QUIRKS.md](../projects/pg-ai-stewards/docs/AGE-QUIRKS.md)
...
| WS5 | pg-ai-stewards (Postgres substrate for agent state, memory, work, model calls) | 🔨 **Phases 2.5 + 2.6a/b/c + 2.7a + 3a + 3b + 2.7b.1 + 2.7b.2 + 2.7b.3 shipped May 4–6.** **2.7b.1:** trigger-driven harvest. **2.7b.2:** bgworker scheduler. **2.7b.3:** per-pass token budget enforcement at enqueue time; verified at 1k/10k/25k/∞ budgets. **2.7b.4 (7-day soak + regenerate_active_md) is next.** | [proposal](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) |
