...
*Updated: 2026-05-06 (pg-ai-stewards Phase 2.7 + 3a.1 shipped same day. **2.7 stack** complete (b.1/b.2/b.3/b.4 — trigger-driven harvest + scheduler + budget + soak-prep). **3a.1: agent corpus imported** — 19 agents from `.github/agents/*.agent.md` (study, dev, journal, lesson, plan, sabbath, ux, etc.) + 20 skills from `.github/skills/*/SKILL.md` into `stewards.agents` / `stewards.skills` / `stewards.agent_tool_perms`. Each agent has 5–17 tool perms. `compose_system_prompt('study', ...)` produces 20K chars. Tolerant YAML parser handles both Copilot list and Claude comma-string `tools` formats. Six malformed `.github/agents/*.agent.md` files were repaired (bare-list missing `tools:` key) — same-bug-same-fix. **Substrate is now usable** — any client (CLI, future MCP) can compose a real agent prompt by family name. docs/architecture.md (466-line reading map). Soak hasn't started; agent import + 2.7b.4 commits not yet made — Michael wants to inspect before pushing. **Next: bulk-tag journals + start the soak; in parallel, design 3c (pipelines).**) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
...
| WS5 | pg-ai-stewards (Postgres substrate for agent state, memory, work, model calls) | 🔨 **Phase 2.7 + 3a.1 shipped May 4–6.** 2.7 stack (substrate-trigger-scheduler-budget-soak-prep) + agent corpus imported (19 agents + 20 skills + 272 tool perms). **Substrate is usable end-to-end** — any client can `compose_system_prompt(family, model, session)` and get a real working agent prompt. **Next: bulk-tag journals + start soak; design 3c (pipelines).** | [proposal](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) |
