...
*Updated: 2026-05-06 (pg-ai-stewards Phase 2.7b.1 shipped — trigger-driven Watchman automation. Pure SQL: `watchman_passes` + `watchman_config` tables, `watchman_pass_start()`, `AFTER UPDATE` trigger on `work_queue` harvests verdicts/findings transactionally. CLI: `watchman pass-now`, `passes`, `pass-detail`, `config show/set`. Inverse hypothesis confirmed (4 synthetic trials); 5-doc real-model verification passed; defensive path validated by a real-world opencode 502. lib.rs foldback complete — sixth `extension_sql_file!`. 2.7b.2 (bgworker scheduler) up next.) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
...
5. **[WS5] pg-ai-stewards** — Postgres-as-AI-substrate. **Phases 2.5 + 2.6a/b/c + 2.7a + 3a + 3b + 2.7b.1 shipped May 4–6.** **v0.2.0:** six SQL files (2-6a/b/c + 2-7a + 3a + 2-7b1) folded into `lib.rs` via `extension_sql_file!`; `init/01-seed-workstreams.sql` extracted post-install (AGE-QUIRKS #9). **3b:** `response_format jsonb` on `stewards.agents`, dry_run_chat injection, `--max-input-chars` 60/40 head/tail. **2.7b.1 (trigger-driven Watchman automation):** `stewards.watchman_passes` + `watchman_config` tables, `watchman_pass_start()` enqueuer, `AFTER UPDATE OF status` trigger on `work_queue` filtered by `payload ? '_watchman_pass_id'` harvests verdicts/findings transactionally with chat completion. CLI: `watchman pass-now|passes|pass-detail|config show/set`. Inverse hypothesis (synthetic 4 trials) confirmed trigger is load-bearing; 5-doc real-model verification passed (1 clean + 4 skipped, 18902/18677 tokens, 7m45s); defensive path validated by real-world opencode 502 mid-pass. **Open:** 2.7b.2 (bgworker scheduler tick), 2.7b.3 (per-pass token budget enforcement), 2.7b.4 (7-day soak + regenerate_active_md), ws6 AGE upstream PRs, 3c–3g. → [proposal](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) · [phases.md](../projects/pg-ai-stewards/phases.md) · [AGE-QUIRKS.md](../projects/pg-ai-stewards/docs/AGE-QUIRKS.md)
...
| WS5 | pg-ai-stewards (Postgres substrate for agent state, memory, work, model calls) | 🔨 **Phases 2.5 + 2.6a/b/c + 2.7a + 3a + 3b + 2.7b.1 shipped May 4–6.** **2.7b.1:** trigger-driven Watchman automation; SQL substrate + `AFTER UPDATE` trigger on `work_queue` harvests verdicts/findings; bgworker stays generic. Inverse hypothesis confirmed; 5-doc real-model verification passed. **2.7b.2 (bgworker scheduler) is next.** | [proposal](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) |
