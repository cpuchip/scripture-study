...
*Updated: 2026-05-07 (pg-ai-stewards 3c stack now complete through 3c.2.5. **3c.2.5: study tools registered.** 5 sql_fn wrappers (`study_search_text`, `study_get`, `study_similar`, `study_citations`, `study_context_for`) + 2 underlying SQL functions (`study_search_text` and `study_get` with line-based pagination), all callable as JSON-Schema-described tools by any non-watchman agent. Blanket `study_*: allow` perm granted across the imported agent corpus (the Copilot tool patterns don't bridge automatically). `tool_defs.expected_result_tokens` + `expected_invocation_tokens` columns added (NULL — populate after observation). Verification: 7 tools registered (was 2); `dry_run_chat('study', 'kimi-k2.6', ...)` now emits 6 tools (5 study + skill) with full JSON Schema preserved through compose_tools. **3c.3 now fully unblocked.** Soak still not flipped on.) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
...
| WS5 | pg-ai-stewards (Postgres substrate for agent state, memory, work, model calls) | 🔨 **Phase 2.7 + 3a.1 + 3c.1 + 3c.2 + 3c.2.5 shipped May 6–7.** Watchman stack + agent corpus + pipelines/work_items + auto-advance + 5 study tools registered. **Next: 3c.3 (first real multi-stage pipeline) — fully unblocked. Soak ready but not started.** | [proposal](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) |
