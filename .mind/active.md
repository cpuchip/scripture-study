...
| WS7 | 1828-illuminated scriptures (1828.ibeco.me) | 🔨 **Backend pivot RATIFIED 2026-05-20 — ready to build phase 1.** Five proposals in `projects/1828-illuminated/.spec/proposals/`, all decisions settled. Headline ratifications: **D-BE-COPYRIGHT option D hybrid** (bcbooks 2013 verse text, strip footnotes + headings on ingest, always-on tabbed-iframe breakout to churchofjesuschrist.org); **D-LP-1** opencode-go/zen as v1 LLM provider (LM Studio reserved for embeddings only, accessed via Dokploy host tunnel in future pgvector phase); **D-LP-2** BYOK + server-side in-memory 24h sliding-TTL session-key model (keys never persisted); **D-LP-4** 200k tokens/day server cap; **D-BE-AUTH** session-required + 10/min/1000/day per-IP, rate-limit errors clearly attributed to 1828 not the provider; **D-SC-2** english tsv + custom archaic-suffix expansion; **D-DICT-1** full 98k 1828 ingest (class-E reach); **D-BE-INTENT-YAML** doc rewrites land in same commit. CLAUDE.md + intent.yaml rewritten now. Thummim batch background process completed 14 verified, $5.76 spent, zero failures (charity self-corrected Moroni 7:46→7:48). Next implementing session: phase 0 (CLAUDE.md updated ✓) → phase 1 (compose file + Postgres + empty Go skeleton + nginx /api proxy). Commits ac687b9 (route fix) + 3997fbc (proposals). | [proposals](../projects/1828-illuminated/.spec/proposals/) · [intent](../projects/1828-illuminated/intent.yaml) · [project CLAUDE.md](../projects/1828-illuminated/CLAUDE.md) |
