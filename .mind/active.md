...
*Updated: 2026-05-07 (pg-ai-stewards 3c.3 v1 shipped + ran end-to-end on real binding question; outcome was instructive failure that surfaced 3 substrate bugs. **What worked:** stage input templating (`render_stage_input`, `{{input.X}}` / `{{stage_results.STAGE.field}}`); 3-stage `study-write` pipeline (outline/draft/review); the agent ACTUALLY USED substrate tools (`study_search_text`, `study_get`, `study_citations`) despite the imported prompt referencing Copilot tools — the JSON Schema bridge was sufficient. **What broke:** (1) `v_is_final` in 3c.2 trigger evaluates to NULL when `loop_stop_reason IS NULL`; `IF NOT NULL` then doesn't take the early-return branch, so trigger advances on intermediate chats; (2) `chat_post_internal` (used by tool_dispatch continuation) doesn't propagate `_work_item_id` markers, so trigger only sees the FIRST chat per stage and rolls up tokens incorrectly; (3) imported agents' `steps=8` is too tight for real tool-using research — plan/study agents step-exhausted on all 3 stages without ever producing synthesis content. **Cost:** ~460K tokens (vs 200K budget) because bug 2 caused massive undercount in token rollup, so budget gate didn't trip. **3c.3.1 is the bug-fix follow-up; 3c.3.2 re-runs after fixes.** Honest framing: 3c.3 v1 surfaced exactly the kind of issues v1 was supposed to find. Substrate is sound; trigger semantics need refinement.) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
...
| WS5 | pg-ai-stewards (Postgres substrate for agent state, memory, work, model calls) | 🔨 **3c.3 v1 ran end-to-end May 7.** Substrate works; agent uses tools; **3 trigger bugs surfaced that prevent real synthesis output.** 3c.3.1 is the fix follow-up. Soak ready but not started. | [proposal](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) |
