...
*Updated: 2026-05-06 (pg-ai-stewards post-3a cleanup: foldback debt cleared → v0.2.0; Phase 3b shipped — `response_format` column on `stewards.agents` + `dry_run_chat` injection + `--max-input-chars` CLI flag with 60/40 head/tail split; AGE-QUIRKS #9 documented; phases.md updated; 2.7b bgworker queued for next session) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
...
5. **[WS5] pg-ai-stewards** — Postgres-as-AI-substrate. **Phases 2.5 + 2.6a/b/c + 2.7a + 3a + 3b shipped May 4–6.** **v0.2.0:** all five SQL files (2-6a/b/c + 2-7a + 3a) folded into `lib.rs`; `init/01-seed-workstreams.sql` extracted post-install (avoids CREATE EXTENSION search_path corruption — AGE-QUIRKS #9). **3b:** `response_format jsonb` column on `stewards.agents`; `dry_run_chat` injects it when non-NULL; `watchman-consolidator` seeded with `{"type":"json_object"}`; `--max-input-chars` flag on CLI with 60/40 head/tail split + elision marker (fixes 120s timeout on big docs). **AGE-QUIRKS.md now 9 entries** (#9: search_path leak). **Open:** 2.7b (Watchman bgworker automation, next session), ws6 AGE upstream PRs (documented, not yet filed), 3c–3g. → [proposal](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) · [phases.md](../projects/pg-ai-stewards/phases.md) · [AGE-QUIRKS.md](../projects/pg-ai-stewards/docs/AGE-QUIRKS.md)
...
| WS5 | pg-ai-stewards (Postgres substrate for agent state, memory, work, model calls) | 🔨 **Phases 2.5 + 2.6a/b/c + 2.7a + 3a + 3b shipped May 4–6. v0.2.0.** Foldback cleared. Model dispatch + Watchman pass (3a) + `response_format` injection + big-doc input trimming (3b). AGE-QUIRKS.md 9 entries. **2.7b bgworker automation next.** | [proposal](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) |
