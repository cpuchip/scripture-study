...
*Updated: 2026-05-06 (pg-ai-stewards Phase 2.7b.1 + 2.7b.2 shipped same day. **2.7b.1:** trigger-driven Watchman automation; `AFTER UPDATE` trigger on `work_queue` harvests verdicts/findings transactionally. **2.7b.2:** bgworker scheduler tick (60s); SQL `watchman_should_fire()` decides pressure/cron/idle, Rust just polls; live verified by real fire 60s after enable. CLI: `watchman pass-now|passes|pass-detail`, `watchman scheduler-status`, `watchman config set --enabled` etc. Seven `extension_sql_file!` foldback. Container at v0.2.0. Two debug catches surfaced during 2.7b.2: read-only `Spi::connect` blocks function-internal writes (use `connect_mut`); Dockerfile `COPY` lists SQL files explicitly (must update when adding `extension_sql_file!`). 2.7b.3 (token budget enforcement) next, then 2.7b.4 (7-day soak).) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
...
5. **[WS5] pg-ai-stewards** — Postgres-as-AI-substrate. **Phases 2.5 + 2.6a/b/c + 2.7a + 3a + 3b + 2.7b.1 + 2.7b.2 shipped May 4–6.** **v0.2.0:** seven SQL files folded into `lib.rs` via `extension_sql_file!` (2-6a/b/c, 2-7a, 3a, 2-7b1, 2-7b2). **2.7b.1:** trigger-driven Watchman; `AFTER UPDATE OF status` on `work_queue` filtered by `payload ? '_watchman_pass_id'` harvests verdicts/findings transactionally. CLI: `watchman pass-now|passes|pass-detail|config show/set`. **2.7b.2:** bgworker scheduler tick (60s); SQL `watchman_should_fire()` decides pressure/cron/idle, Rust polls via `Spi::connect_mut`; `last_sched=None` startup forces immediate first check. 9 SQL decision-trial verification + live fire 60s after enable (`actor='scheduler'`, `trigger='pressure'`). CLI gained `watchman scheduler-status` for "why not firing?" debugging. Two build-time catches: `Spi::connect` is read-only (must use `connect_mut`); Dockerfile `COPY` lists SQL files explicitly. **Open:** 2.7b.3 (per-pass token budget enforcement), 2.7b.4 (7-day soak + regenerate_active_md), ws6 AGE upstream PRs, 3c–3g. → [proposal](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) · [phases.md](../projects/pg-ai-stewards/phases.md) · [AGE-QUIRKS.md](../projects/pg-ai-stewards/docs/AGE-QUIRKS.md)
...
| WS5 | pg-ai-stewards (Postgres substrate for agent state, memory, work, model calls) | 🔨 **Phases 2.5 + 2.6a/b/c + 2.7a + 3a + 3b + 2.7b.1 + 2.7b.2 shipped May 4–6.** **2.7b.1:** trigger-driven Watchman automation. **2.7b.2:** bgworker scheduler tick (60s); SQL decides pressure/cron/idle; live fire verified end-to-end. **2.7b.3 (token budget enforcement) is next.** | [proposal](../.spec/proposals/pg-ai-stewards-phase-2-5-generic-substrate.md) |
