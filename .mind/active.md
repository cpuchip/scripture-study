...
*Updated: 2026-05-07 (pg-ai-stewards image rebuild — **foldback chain is now image-resident**. All 14 SQL files (2-6a/b/c, 2-7a, 3a, 2-7b1–b4, 3c1, 3c2, 3c2-5, 3c3, 3c3-1) bake into `pg_ai_stewards--0.2.0.sql` at build time. Smoke test on a fresh ephemeral container caught one drift bug: `3a-watchman-pass.sql` default `watchman-consolidator '*'` variant was missing `response_format` in its VALUES tuple — masked because the live DB had been UPDATE-patched after the column was added. Fixed in source, re-built, re-smoked clean (4 agents / 7 tool_defs / 2 pipelines / 1 watchman_config on a virgin DB). Live `pg-ai-stewards-dev` container left running on the old image — `pgdata` named volume is persistent, so next natural restart picks up the new image with soak history intact. **Lesson:** live-applied SQL drifts from repo source; only `CREATE EXTENSION` on a fresh database surfaces structural mismatches that ON CONFLICT-DO-UPDATE hides. Earlier today: 3c.3.1 + 3c.3.2 shipped, substrate produced first real meta-study (`study/two-triplets-one-ascent.md`), Watchman soak started.) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
