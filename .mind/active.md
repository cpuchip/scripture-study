...
*Updated: 2026-05-08 (pg-ai-stewards overnight session — **kimi-tuned study prompt validated end-to-end**. Phases 3c.3.3 (importer reads `model_match` from frontmatter, enabling per-model agent variants) and 3c.3.4 (4-way multi-model voice experiment on FtC/WtL: kimi+base, kimi+tuned-no-corpus, qwen+base, kimi+tuned-with-corpus). Run #4 is the strongest — opens with Thomas's question, claim-style headers, anti-symmetry framing, and **actively verified itself** by catching+removing two fabricated quotes from its own draft via `study_search_text`. The "verification claims must be tool-grounded" rule of the kimi-tuned prompt operating at full strength. Six qwen-specific signatures identified (run #3) for a future `.stewards/qwen-3.6/` variant. One bug found+patched: `agent_tool_perms` has no source provenance, so 3c.3.3's reimport wiped the 3c.2.5 `study_*: allow` broadcast. Workaround: added `study_*` to both study agent frontmatters; daytime followup is the provenance column. 3c.4 (gospel-engine HTTP tools) deferred — neither pg_net nor http extension in pgvector pg18 base, and Rust bgworker changes too risky unsupervised. Total experiment spend ~$0.35 across 4 runs. Soak ran 6 passes through the night (00:17 → 05:24 UTC) untouched. Comparison memo at `study/.scratch/two-triplets-comparison-2026-05-08/`. Earlier yesterday: image rebuild made foldback chain image-resident; 3c.3.1 + 3c.3.2 shipped first real meta-study.) · Previous: [archive/active-2026-04-23.md](archive/active-2026-04-23.md)*
