...
**Search results are pointers, not sources. Memory is not a source.** Use search tools to *find* what to study. Use `Read` to *actually study it*. Use `mcp__webster__webster_define` to *understand the language*. Each tool has a role; none replaces the others.
...
- `mcp__gospel-engine-v2__gospel_search` for keyword/phrase/semantic/combined search across scriptures, conference, manuals
- `mcp__webster__define` / `mcp__webster__webster_define` for historical word meanings
- `mcp__exa-search__web_search_exa` for external scholarly context
...
- For EVERY scripture you plan to quote, `Read` the actual chapter markdown from `gospel-library/`
- For EVERY conference talk you plan to cite, `Read` the actual talk file
...
For **N** citations in your output, perform at least **N** `Read` calls (or equivalent verified tool reads — e.g., a `mcp__byu-citations__byu_citations` lookup, a `mcp__webster__webster_define` call, a `mcp__gospel-engine-v2__gospel_get` for the full source).

If you cite 8 scriptures and 2 conference talks, you should have called `Read` (or `gospel_get`) at least 10 times on actual source files.
...
|------------------------------|---------------------------------------|
| `Read` on a gospel-library markdown file | `gospel_search` excerpts |
| `Read` on a conference talk file | gospel_search semantic snippets |
| `Read` on a manual chapter | gospel_search talk summaries |
| | Any search result labeled `[AI SUMMARY]` |
...
- Is there an existing personal-application document this connects to?
...
- [ ] No quotes generated from memory — every `"quoted text"` has a corresponding `Read` call
...
- [ ] Files claimed to exist are verified with `Glob` or `Bash ls`
