...
Your `agent_id` is your **session lane** — e.g. `lane:pg-ai-stewards`, `lane:general-workspace`. Register once per session (idempotent):

```
mcp__pg-ai-stewards__a2a_register(agent_id="lane:<your-lane>", display_name="…", kind="session", lane="<your-lane>")
```
...
1. **Check your inbox** on engagement — `a2a_inbox(agent_id="lane:<you>")`. It returns two panes:
   - **notes** — async messages to you. Act on them, then `a2a_note_clear(recipient="lane:<you>")`.
   - **todos** — open work assigned to you, each with its blocking question (if any).
2. **Claim** before working — `a2a_claim(work_item_id=…, claimer="lane:<you>")`. The claim is a lock: if you get `claimed:false`, another agent owns it — move on. A successful claim returns the full ticket/`spec`.
...
a2a_submit(
  ...
  owner="lane:<you>")     # so you get the questions + the receipt
```
...
For a lighter touch (no tracked task, no receipt), just leave a note: `a2a_note(recipient="…", body="…", sender="lane:<you>")`.
...
2. `a2a_submit(assignee="agy", title="Say hello", spec={outcome:"greet the engine"}, owner="lane:<you>")`.
