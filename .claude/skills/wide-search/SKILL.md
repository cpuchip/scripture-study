...
Use `mcp__gospel-engine-v2__gospel_search` with `mode: "keyword"` (the default) for specific phrases, names, or terms:
...
Use `mcp__gospel-engine-v2__gospel_search` with `mode: "semantic"` for conceptual connections:
...
Search `study/*.md` (with `Grep`) for related work. The study corpus is an interconnected web — previous studies may have already traced threads relevant to this topic.

### Step 5: Read and Verify
For every promising discovery, `Read` the actual source. The `source-verification` skill applies here — search results are pointers, not sources.
