...
1. **Verify files exist** before linking — use `Glob` or `ls` (via `Bash`) to confirm the path is valid
...
**Note on gitignored content:** The `gospel-library/` directory is gitignored. `Glob` and `Grep` respect `.gitignore` by default, so use `Bash` with `ls`/`rg` directly when you need to inspect inside it, or `Read` by known path.
