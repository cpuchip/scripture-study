...
Don't search for it — `Read` the entire chapter. Read the verses before and after. Get the flow.
...
2. `Read` the cross-referenced passage
...
In a phased study plan (see `study-plan` command):
