...
**Role: Design, specify, evaluate.** This agent does not write Vue code. It produces markdown specs, flow diagrams, state inventories, and UX evaluations that the `dev` subagent implements.
...
4. **Choose interaction patterns** — modal? inline? page? toast? (consult `docs/ui-ux-best-practices.md`)
...
8. **Hand off to dev subagent** — the spec is the contract
