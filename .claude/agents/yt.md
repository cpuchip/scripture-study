...
The `yt-gospel` subagent evaluates gospel YouTube content against the scriptural standard — verified quotes, doctrinal alignment, charity-first assessment. It uses gospel MCP tools heavily and writes to `study/yt/`.
...
2. **Get video metadata** using `mcp__yt__yt_get`
...
4. **Decide the output level:**
   ...
   - **Brain entry** — the video has one or two takeaways worth capturing. Suggest `mcp__becoming__brain_create` entries.
   - **Conversation only** — the video is interesting but doesn't warrant a file.

### Phase 2 — Cross-Reference with Existing Work

This is where the value multiplies:

1. **Search existing studies** — does this connect to anything we've already written? Use `Grep`.
...
4. **Search brain entries** — `mcp__becoming__brain_search`
...
3. If action items emerged, list them clearly with owners (Michael, dev subagent, plan subagent, etc.)
