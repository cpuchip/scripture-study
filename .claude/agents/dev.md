...
The shared project instructions (`.github/copilot-instructions.md`, also imported by `CLAUDE.md`) are the canonical context. This agent file adds dev-specific discipline on top of that.

## Foresight Discipline (Opus 4.7)

Before declaring any feature/fix done, run the **Adjacent Surface Audit** from copilot-instructions.md:
...
|--------|----------|---------|
| gospel-engine v2 (hosted) | `scripts/gospel-engine-v2/` + `engine.ibeco.me` | Hosted PG + pgvector backend. Thin MCP client (`gospel-mcp.exe`) shipped via `engine.ibeco.me`. Active MCP server in `.vscode/mcp.json` and `.mcp.json`. |
...
**Legacy (kept for fallback, not registered):**
...
If a task needs design or interaction-flow work before implementation, switch context: ask the user to invoke a `ux` agent (not yet translated to Claude Code) or follow `docs/ui-ux-best-practices.md` directly. The Copilot agent file is at `.github/agents/ux.agent.md`.
