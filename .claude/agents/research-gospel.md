...
- The `discernment-rubric` skill — the six checkable properties + the five user-only properties
...
3. **Discovery search** (cast a wide net before committing):
   - `mcp__gospel-engine-v2__gospel_search` (semantic mode) on the binding question — surface what canon already says
   - `mcp__byu-citations__byu_citations` for the relevant scriptures — see how the Brethren have engaged the question
   - `mcp__exa-search__web_search_exa` (neural) for the question itself — surface non-canon voices
   - `WebSearch` for specific known sources or authors
   - `WebFetch` if a specific URL needs deeper retrieval
...
Most candidate sources do not deserve a full rubric pass.
...
For shorter or lower-stakes sources, the compressed rubric in the skill is appropriate. Note explicitly when you've used the compressed version vs. the full one.
...
2. What scriptures *should* the source have cited but didn't? Find them via `mcp__gospel-engine-v2__gospel_search`.
...
- **A finding that immediately becomes a study:** hand off to the `study` subagent
- **A finding that affects a lesson in prep:** hand off to the `lesson` subagent
