...
**Start with discovery, not recall.** Before drafting, run at least one `mcp__gospel-engine-v2__gospel_search` (semantic or combined) on the lesson's binding question or central scripture. Per Anthropic's 4.7 guide, this model uses tools less by default — reach for them explicitly. Semantic search surfaces conference talks and cross-references your memory wouldn't.

The rhythm:
1. `mcp__gospel-engine-v2__gospel_search` (semantic or combined) on the lesson's binding question → note paths in scratch file
2. `Read` each key scripture chapter → write verified quotes + teaching insights to scratch file
3. Follow footnotes — they often lead to the most powerful cross-references
4. `Read` each discovered source → write to scratch file
...
2. **Class check:** Who is in this class? What do they already know? What do they struggle with? A lesson that's perfect for gospel scholars might lose new members, and vice versa.
...
4. **Question quality:** Transform every yes/no question into an open question. "Is faith important?" → "When has acting on faith changed what you could see?"
5. **Application check:** Does this lesson end with knowledge or with direction?
...
7. **Posture check:** Is this lesson designed to teach the class, or to impress them?
...
**Open with a story, not a topic sentence.** President Monson's children perked up from sleep when he began a story. A lesson that opens "Today we're going to talk about faith" starts at zero. A lesson that opens with Alma hiding in the hill country, writing by whatever light he had, the words of a dead man, starts with the class already inside the text. Use names, places, specific detail.

**Therefore and but, not "and then."** A lesson where principle A *therefore* leads to principle B, *but* the class discovers a complication in principle C, has the shape of discovery. Questions should create "but" moments: "We said faith leads to action. But what about the times you acted in faith and nothing seemed to happen?"
...
**Less is more.** A focused lesson on one principle, explored deeply, beats a survey of five topics. The preparation should make the teacher an expert who only uses 20% of what they know.

**Trust the silence.** If you ask a good question and nobody answers for ten seconds, that silence is not a failure. That's the class thinking.

**The teacher's testimony is the bridge.** Help them find where to testify — not as an afterthought, but at the exact moment when the principle meets lived experience.

**Cross-reference past lessons and studies.** When you spot a connection, name it.
