...
Plan a multi-phase scripture study on: **$ARGUMENTS**

If `$ARGUMENTS` is empty, ask the user for the broad topic and the driving question before doing anything.

If a more specialized study workflow exists, consider invoking it via `Agent(subagent_type=study, ...)`. Otherwise proceed with the steps below.
...
- What do we already know? (Check `study/*.md` with `Glob`/`Grep` for existing related studies)
...
Create a directory and plan file at `study/{topic-slug}/00_plan.md`:

```markdown
# Study Plan: {Study Topic}
...
After the plan is reviewed and approved by the user, start Phase 1 using the appropriate skill (`deep-reading` or `wide-search`). Create the intermediate notes file and begin working. The `quote-log` skill is mandatory across phases — externalize quotes as you read so they survive context compaction.
